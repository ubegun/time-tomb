...
There are **two** corpora and only one of them is timed. ``bench/corpus`` is the
timed one: every latency, cold-start and stage-split figure published is a
property of its seven documents, so it is frozen. ``bench/corpus-quality`` is
larger, carries the 64-query labelled set - categorised, phrasing-varied and
split into held-out and tuning - and is read only by the untimed scoring. The
labelled set exists because eight queries cannot separate two profiles: one miss
there is a 12.5 point swing.
...
Usage:
    ...
    python bench.py --emit-corpus-quality bench/corpus-quality
    python bench.py --all --corpus bench/corpus --corpus-quality bench/corpus-quality
...
# --- the quality corpus (phase 2) -------------------------------------------
#
# A second body, generated by the same machinery under a different seed. The
# timed corpus above is frozen: every latency figure in BENCHMARKS.md is a
# property of its seven documents and its 48 / 66 / 256 chunks, so growing it
# would silently move numbers that have nothing to do with this run. The
# labelled set that can actually separate two profiles needs about ninety
# planting slots and 6 x 6 = 36 is not enough, so it gets its own body.
#
# Nothing timed ever touches this corpus. It exists only for the untimed
# quality, token-budget and probe scoring.

# Schema of the shipped ground-truth document. 1 is the timed corpus, frozen
# with it; 2 is the labelled quality corpus, which carries the extra fields.
CORPUS_SCHEMA_VERSION = 1

QUALITY_CORPUS_SEED = 20260721      # must differ from CORPUS_SEED
QUALITY_CORPUS_NOTES = 24
QUALITY_SECTIONS_PER_NOTE = 8       # 24 x 8 = 192 slots for ~92 plantings
QUALITY_CORPUS_SCHEMA_VERSION = 2
QUALITY_CORPUS_DIR = BENCH_DIR / "corpus-quality"

# 36 words cannot carry 24 notes without visible repetition. The extension is
# filler vocabulary only: no word here is a family noun, a device noun or a
# marker, and "beacon" stays absent from both corpora.
_WORDS_EXTRA = (
    "alcove ambergris ashlar bramble brazier calyx cinder claret coppice "
    "culvert dormer drayage ember estuary fallow flotsam garret girder hackle "
    "hoarfrost ingot joist kestrel limber lintel maquette moraine newel oakum "
    "parapet pergola quernstone rampart sallow scrivener shingle tallow thatch "
    "tumbrel undertow vestibule wainscot yarrow zephyr"
).split()

QUALITY_WORDS = _WORDS + _WORDS_EXTRA

CATEGORIES: Tuple[str, ...] = ("single-hop", "multi-doc", "vague", "unanswerable")
PHRASINGS: Tuple[str, ...] = ("natural", "terse", "verbose", "misspelled")
SPLITS: Tuple[str, ...] = ("held-out", "tuning")
# One query in four is tuning. Assigned over a (category, phrasing, id) ordering
# so both axes are stratified by construction rather than by luck.
TUNING_EVERY = 4
HELD_OUT = "held-out"
TUNING = "tuning"

# A per-category number computed from fewer than this many queries is rendered
# with an "indicative only" marker. Hard guard 6: no claim the split cannot
# carry.
INDICATIVE_MIN_QUERIES = 8

# The confusable families, generalised from the timed corpus's strict pairs to
# 2-4 members. A family noun is shared; the device noun is what distinguishes
# the topics, and members of one family are planted into disjoint note sets, so
# a retriever that matches the rare family token alone lands on the wrong part
# of the corpus. Fourteen of the eighteen families have three or more members.
_QUALITY_FAMILIES: Tuple[Tuple[str, Tuple[str, ...]], ...] = (
    ("brenmoor", ("governor", "manifold", "damper", "buffer")),
    ("calvestry", ("escapement", "diaphragm", "wickstay", "rail")),
    ("dunwidge", ("gasket", "flywheel", "bushing", "trunnion")),
    ("eldrache", ("gudgeon", "ratchet", "pawl", "cleat")),
    ("fennimore", ("shackle", "turnbuckle", "grommet", "ferrule")),
    ("gostrelle", ("collet", "mandrel", "arbor", "tappet")),
    ("halbrook", ("rocker", "camlock", "keyway")),
    ("ivestrand", ("splinebox", "drumhead", "brakeshoe")),
    ("jarrowmede", ("clevis", "yoke", "stanchion")),
    ("kelbourne", ("bracket", "sleeve", "liner")),
    ("lomasque", ("hasp", "spigot", "bonnet")),
    ("mardrenne", ("bung", "sluice", "weir")),
    ("norvellis", ("penstock", "scupper", "baffle")),
    ("orthwaine", ("vane", "impeller", "volute")),
    ("pyrrhault", ("diffuser", "nozzle")),
    ("quennisford", ("venturi", "orifice")),
    ("ruskelwaite", ("throttle", "regulator")),
    ("tavenmoor", ("actuator", "linkage")),
)

# Eight device nouns that occur nowhere in either corpus. Paired with a family
# noun the corpus is full of, so the retriever answers confidently and wrongly -
# which is the failure latency cannot see. Asserted absent from the generated
# bytes in test_bench_quality.py.
_ABSENT_DEVICES: Tuple[str, ...] = (
    "escutcheon", "quillon", "mullion", "barbican",
    "trefoil", "corbel", "finial", "voussoir",
)
# Drawn from the smaller families on purpose: an unanswerable query reuses a
# family noun, so taking one from a four-member family would push that family to
# five and break the 2-4 contract the confusable families are held to.
_UNANSWERABLE_FAMILIES: Tuple[str, ...] = (
    "halbrook", "ivestrand", "jarrowmede", "kelbourne",
    "pyrrhault", "quennisford", "ruskelwaite", "tavenmoor",
)

ASPECT_ORDER: Tuple[str, ...] = (
    "pressure", "interval", "temperature", "tool",
    "order", "cause", "torque", "clearance",
)

# Each aspect carries three *parts*. A single-hop topic plants part 0 in its one
# document; a multi-doc topic plants parts 0..n-1 in n documents, so the parts
# are different pieces of one answer rather than the same sentence copied - the
# category name has to be true of the data, not only of the label.
_QUALITY_ASPECTS: Dict[str, Dict[str, object]] = {
    "pressure": {
        "values": ("47", "52", "61", "38", "74", "43", "66"),
        "unit": "kilopascal",
        "parts": (
            "The {f} {d} holds a cold-start pressure of {v} kilopascal. A crew "
            "reads the {f} {d} before the first pass of a shift, because a {d} "
            "that has drifted more than four kilopascal below that figure will "
            "not seat.",
            "Setting the cold-start pressure on a {f} {d} is a two-stage job: "
            "bring it to {v} kilopascal against a cold line, let it stand a "
            "quarter of an hour, then bring it back. A {f} {d} set in one stage "
            "reads correct and falls away before the shift is out.",
            "One exception is written up for the {f} {d}: on a line drained and "
            "refilled the same day the {v} kilopascal figure is read as an upper "
            "bound rather than a target, and the {d} is left a little under it.",
        ),
        "query": {
            "natural": "what pressure does the {f} {d} hold at cold start",
            "terse": "{f} {d} cold start pressure",
            "verbose": "I am going through the shift start-up sheet and I "
                       "cannot remember what the cold-start pressure on the {f} "
                       "{d} is supposed to be, could someone tell me the figure",
            "misspelled": "what presure does the {f} {d} hold at cold start",
        },
        "de": "welchen Druck h\u00e4lt der {n} beim Kaltstart",
        "ru": "\u043a\u0430\u043a\u043e\u0435 \u0434\u0430\u0432\u043b\u0435\u043d\u0438\u0435 \u0434\u0435\u0440\u0436\u0438\u0442 {n} \u043f\u0440\u0438 \u0445\u043e\u043b\u043e\u0434\u043d\u043e\u043c \u043f\u0443\u0441\u043a\u0435",
        "uk": "\u044f\u043a\u0438\u0439 \u0442\u0438\u0441\u043a \u0442\u0440\u0438\u043c\u0430\u0454 {n} \u043f\u0440\u0438 \u0445\u043e\u043b\u043e\u0434\u043d\u043e\u043c\u0443 \u043f\u0443\u0441\u043a\u0443",
    },
    "interval": {
        "values": ("nine hundred", "twelve hundred", "four hundred",
                   "two thousand", "six hundred", "fifteen hundred", "eight hundred"),
        "unit": "cycles",
        "parts": (
            "The {f} {d} is re-shimmed every {v} cycles. Shimming a {f} {d} "
            "early wastes a wear allowance that is measured over the whole "
            "service interval and not over a single week.",
            "Counting the {v} cycles for a {f} {d} starts at the last full "
            "strip-down, never at the last inspection. A count restarted at an "
            "inspection quietly shortens the interval and hides the wear it was "
            "meant to reveal.",
            "A {f} {d} that has been re-shimmed twice inside one interval is "
            "retired rather than shimmed a third time, whatever the {v}-cycle "
            "figure allows.",
        ),
        "query": {
            "natural": "how often is the {f} {d} re-shimmed",
            "terse": "{f} {d} re-shim interval",
            "verbose": "quick question before I raise the job card, how many "
                       "cycles are we meant to run before the {f} {d} gets "
                       "re-shimmed again",
            "misspelled": "how offen is the {f} {d} re-shimmed",
        },
        "de": "wie oft wird der {n} neu unterlegt",
        "ru": "\u043a\u0430\u043a \u0447\u0430\u0441\u0442\u043e \u043e\u0431\u0441\u043b\u0443\u0436\u0438\u0432\u0430\u044e\u0442 {n}",
        "uk": "\u044f\u043a \u0447\u0430\u0441\u0442\u043e \u043e\u0431\u0441\u043b\u0443\u0433\u043e\u0432\u0443\u044e\u0442\u044c {n}",
    },
    "temperature": {
        "values": ("sixty-two", "fifty-five", "seventy-one", "forty-eight",
                   "sixty-six", "fifty-nine", "seventy-four"),
        "unit": "degrees",
        "parts": (
            "The {f} {d} must stay under {v} degrees at the outlet. Above that "
            "the {f} {d} begins to shed its facing, and the loss is not "
            "recovered by cooling it back down afterwards.",
            "Outlet temperature on a {f} {d} is read at the far flange and not "
            "at the body: the body runs several degrees cooler, so a reading "
            "taken there will pass a {d} that is already over {v}.",
            "A {f} {d} carrying a replacement facing is held ten degrees under "
            "the {v}-degree figure for its first hundred hours, because a fresh "
            "facing sheds at a lower temperature than a bedded one.",
        ),
        "query": {
            "natural": "what temperature limit applies to the {f} {d}",
            "terse": "{f} {d} outlet temperature limit",
            "verbose": "we have been seeing warm readings all week and I want "
                       "to check the paperwork, what is the outlet temperature "
                       "the {f} {d} is not allowed to go above",
            "misspelled": "what temperture limit applies to the {f} {d}",
        },
        "de": "welche Temperaturgrenze gilt f\u00fcr den {n}",
        "ru": "\u043a\u0430\u043a\u043e\u0439 \u043f\u0440\u0435\u0434\u0435\u043b \u0442\u0435\u043c\u043f\u0435\u0440\u0430\u0442\u0443\u0440\u044b \u0443 {n}",
        "uk": "\u044f\u043a\u0430 \u043c\u0435\u0436\u0430 \u0442\u0435\u043c\u043f\u0435\u0440\u0430\u0442\u0443\u0440\u0438 \u0443 {n}",
    },
    "tool": {
        "values": ("quarter-inch", "half-inch", "three-eighths", "six-millimetre",
                   "ten-millimetre", "eighth-inch", "twelve-millimetre"),
        "unit": "offset key",
        "parts": (
            "The {f} {d} is released with a {v} offset key. Prying a {f} {d} "
            "with a screwdriver deforms the latch seat, which is the one part "
            "of the assembly with no spare.",
            "The {v} offset key is turned against the stop and never past it; a "
            "{f} {d} released past the stop re-latches out of register and has "
            "to be stripped before it can be reset.",
            "Where no {v} key is to hand, a {f} {d} may be released by backing "
            "off its retaining collar instead. It is slower, and it is the only "
            "sanctioned substitute.",
        ),
        "query": {
            "natural": "what tool releases the {f} {d}",
            "terse": "{f} {d} release tool",
            "verbose": "I have the {f} {d} stripped down in front of me and I "
                       "am not sure which tool is the right one to release it "
                       "with, what should I be reaching for",
            "misspelled": "what tool realeases the {f} {d}",
        },
        "de": "welches Werkzeug l\u00f6st den {n}",
        "ru": "\u043a\u0430\u043a\u0438\u043c \u0438\u043d\u0441\u0442\u0440\u0443\u043c\u0435\u043d\u0442\u043e\u043c \u0441\u043d\u0438\u043c\u0430\u044e\u0442 {n}",
        "uk": "\u044f\u043a\u0438\u043c \u0456\u043d\u0441\u0442\u0440\u0443\u043c\u0435\u043d\u0442\u043e\u043c \u0437\u043d\u0456\u043c\u0430\u044e\u0442\u044c {n}",
    },
    "order": {
        "values": ("outboard", "upper", "lower", "port", "starboard",
                   "forward", "after"),
        "unit": "port",
        "parts": (
            "The {f} {d} is bled {v} first, then centre, then inboard. Bleeding "
            "a {f} {d} in the reverse order traps a pocket at the centre port "
            "that no amount of further bleeding clears.",
            "The {v}-first order for a {f} {d} holds for a cold line. On a warm "
            "line the centre port is taken first, and the {d} is then bled {v} "
            "and inboard in the usual way.",
            "A {f} {d} fed from a shared header is bled with the header "
            "isolated; leaving it open lets the {v} port draw back while the "
            "inboard port is still being worked.",
        ),
        "query": {
            "natural": "what is the bleed order for the {f} {d}",
            "terse": "{f} {d} bleed order",
            "verbose": "before I start bleeding this thing, what is the correct "
                       "order to bleed the {f} {d} in, I do not want to trap a "
                       "pocket at the centre port again",
            "misspelled": "what is the bleeed order for the {f} {d}",
        },
        "de": "in welcher Reihenfolge wird der {n} entl\u00fcftet",
        "ru": "\u0432 \u043a\u0430\u043a\u043e\u043c \u043f\u043e\u0440\u044f\u0434\u043a\u0435 \u043f\u0440\u043e\u043a\u0430\u0447\u0438\u0432\u0430\u044e\u0442 {n}",
        "uk": "\u0443 \u044f\u043a\u043e\u043c\u0443 \u043f\u043e\u0440\u044f\u0434\u043a\u0443 \u043f\u0440\u043e\u043a\u0430\u0447\u0443\u044e\u0442\u044c {n}",
    },
    "cause": {
        "values": ("dry", "cold", "over-filmed", "unbedded", "proud",
                   "against the grain", "without a witness mark"),
        "unit": "seating",
        "parts": (
            "The {f} {d} blisters when it is seated {v}. A {f} {d} bedded with "
            "the specified film will not blister even after several hundred "
            "reversals at full stroke.",
            "Blistering on a {f} {d} shows first as a matte ring at the outer "
            "third. Caught there, a {d} seated {v} can be re-bedded; past that "
            "the facing is scrap.",
            "A {f} {d} that has blistered once is not returned to the same "
            "station, because whatever seated it {v} is a property of the "
            "station and not of the part.",
        ),
        "query": {
            "natural": "what causes the {f} {d} to blister",
            "terse": "{f} {d} blistering cause",
            "verbose": "the facing on ours has gone matte around the outer "
                       "third and I am trying to work out why, what actually "
                       "causes a {f} {d} to blister in service",
            "misspelled": "what casues the {f} {d} to blister",
        },
        "de": "wodurch bekommt der {n} Blasen",
        "ru": "\u043f\u043e\u0447\u0435\u043c\u0443 \u043d\u0430 {n} \u043f\u043e\u044f\u0432\u043b\u044f\u044e\u0442\u0441\u044f \u043f\u0443\u0437\u044b\u0440\u0438",
        "uk": "\u0447\u043e\u043c\u0443 \u043d\u0430 {n} \u0437\u2019\u044f\u0432\u043b\u044f\u044e\u0442\u044c\u0441\u044f \u0431\u0443\u043b\u044c\u0431\u0430\u0448\u043a\u0438",
    },
    "torque": {
        "values": ("eleven", "nineteen", "twenty-four", "seven",
                   "thirty-two", "fifteen", "twenty-eight"),
        "unit": "newton metres",
        "parts": (
            "The {f} {d} is tensioned to {v} newton metres, in two passes. A "
            "{f} {d} taken to full torque in one pass reads correct on the "
            "wrench and is still slack across the far side.",
            "The two passes on a {f} {d} are taken at half figure and full "
            "figure, working across the diagonal; a {d} tensioned around the "
            "circle reaches {v} newton metres at every point and still pulls "
            "oval.",
            "A {f} {d} that has been tensioned hot is re-checked cold at {v} "
            "newton metres before it is signed off, and the cold reading is the "
            "one that is written on the card.",
        ),
        "query": {
            "natural": "how is the {f} {d} tensioned",
            "terse": "{f} {d} tensioning torque",
            "verbose": "I want to get this right first time rather than have it "
                       "come back, how is the {f} {d} supposed to be tensioned "
                       "and to what figure",
            "misspelled": "how is the {f} {d} tensionned",
        },
        "de": "wie wird der {n} gespannt",
        "ru": "\u0441 \u043a\u0430\u043a\u0438\u043c \u043c\u043e\u043c\u0435\u043d\u0442\u043e\u043c \u0437\u0430\u0442\u044f\u0433\u0438\u0432\u0430\u044e\u0442 {n}",
        "uk": "\u0437 \u044f\u043a\u0438\u043c \u043c\u043e\u043c\u0435\u043d\u0442\u043e\u043c \u0437\u0430\u0442\u044f\u0433\u0443\u044e\u0442\u044c {n}",
    },
    "clearance": {
        "values": ("nought point four", "nought point six", "one point two",
                   "nought point nine", "two point one", "one point five",
                   "nought point three"),
        "unit": "millimetres",
        "parts": (
            "The {f} {d} takes a running clearance of {v} millimetres. A {f} "
            "{d} closed up tighter than that picks up on the first warm cycle "
            "and cannot be freed again without a strip-down.",
            "Running clearance on a {f} {d} is measured cold with the shaft at "
            "rest; measured turning, a {d} reads under the {v} millimetre "
            "figure and gets closed up further by mistake.",
            "A {f} {d} running in a dusty bay is set two tenths over the {v} "
            "millimetre figure, and the wider setting is written on the card so "
            "the next fitter does not correct it back.",
        ),
        "query": {
            "natural": "what running clearance does the {f} {d} take",
            "terse": "{f} {d} running clearance",
            "verbose": "we are setting one up from cold and I want to confirm "
                       "the number before it goes back together, what running "
                       "clearance should the {f} {d} be left with",
            "misspelled": "what running clearence does the {f} {d} take",
        },
        "de": "welches Laufspiel hat der {n}",
        "ru": "\u043a\u0430\u043a\u043e\u0439 \u0440\u0430\u0431\u043e\u0447\u0438\u0439 \u0437\u0430\u0437\u043e\u0440 \u0443 {n}",
        "uk": "\u044f\u043a\u0438\u0439 \u0440\u043e\u0431\u043e\u0447\u0438\u0439 \u0437\u0430\u0437\u043e\u0440 \u0443 {n}",
    },
}

# A vague query keeps both invented nouns - so the label stays determinate - and
# drops the aspect, which is what makes it underspecified. "Real phrasings"
# lives on the query side (design decision 3): the corpus stays generated.
_VAGUE_QUERY: Dict[str, str] = {
    "natural": "anything I should know about the {f} {d}",
    "terse": "{f} {d} notes",
    "verbose": "someone asked me about the {f} {d} the other day and I realised "
               "I do not actually know what we have written down about it, what "
               "is there",
    "misspelled": "anythin I should know about the {f} {d}",
}

# Same construction as NO_ANSWER_QUERY on the timed corpus: a family noun the
# corpus is full of, a device noun that occurs nowhere in it.
_UNANSWERABLE_QUERY: Dict[str, str] = {
    "natural": "what clearance does the {f} {d} take when it is refaced",
    "terse": "{f} {d} refacing clearance",
    "verbose": "I have been asked to reface one of these and I cannot find the "
               "figure written down anywhere, what clearance is a {f} {d} "
               "supposed to end up with",
    "misspelled": "what clearence does the {f} {d} take when it is refaced",
}

# The category cycle. Fourteen entries repeated four times over the 56
# answerable topics gives exactly 24 / 20 / 12, and spreads each category across
# every family rather than letting one family be entirely one category.
_CATEGORY_CYCLE: Tuple[str, ...] = (
    "single-hop", "multi-doc", "single-hop", "vague", "multi-doc",
    "single-hop", "multi-doc", "single-hop", "vague", "multi-doc",
    "single-hop", "multi-doc", "single-hop", "vague",
)


def _relevant_count(category: str, ordinal: int) -> int:
    """How many documents this topic is planted into.

    24 x 1 + 10 x 2 + 10 x 3 + 6 x 1 + 6 x 2 = 92 plantings, inside 192 slots.
    """
    if category == "single-hop":
        return 1
    if category == "multi-doc":
        return 2 + (ordinal % 2)
    if category == "vague":
        return 1 + (ordinal % 2)
    return 0
...
def _assign_notes(topics: List[Dict[str, object]], total_notes: int) -> None:
    """Least-loaded placement, family members kept document-disjoint.

    Deterministic: fixed topic order, ties broken by note index. The
    family-disjointness is the property that makes the confusable families do
    any work - two topics sharing a family noun must not be answerable from the
    same document, or matching the family alone would be enough.
    """
    loads = [0] * total_notes
    taken: Dict[str, set] = {}
    for topic in topics:
        family = str(topic["family"])
        want = len(topic["facts"])  # type: ignore[arg-type]
        if not want:
            topic["notes"] = ()
            continue
        used = taken.setdefault(family, set())
        candidates = sorted(
            (index for index in range(total_notes) if index not in used),
            key=lambda index: (loads[index], index),
        )
        if len(candidates) < want:
            raise RuntimeError(
                "family %s cannot be placed into %d disjoint documents" % (family, want)
            )
        chosen = sorted(candidates[:want])
        for index in chosen:
            loads[index] += 1
            used.add(index)
        topic["notes"] = tuple(chosen)


def _assign_splits(topics: List[Dict[str, object]]) -> None:
    """Stratified, deterministic held-out / tuning partition.

    Ordered by category, then by phrasing, then by id, and every fourth entry in
    that order is tuning. The category counts are all multiples of four, so that
    much alone makes the split exactly a quarter of every category - but it
    would land on the same phrasing within each category and skew the other
    axis. The phrasing order is therefore **rotated by the category's own
    position**, which costs nothing and makes both axes come out exact.

    Recorded in the ground-truth document and stable across a rebuild: a split
    that moves when the corpus is regenerated invalidates every comparison
    against a previous run.
    """
    category_order = sorted({str(topic["category"]) for topic in topics})

    def key(index: int):
        category = str(topics[index]["category"])
        rotation = category_order.index(category)
        phrasing = PHRASINGS.index(str(topics[index]["phrasing"]))
        return (
            category,
            (phrasing - rotation) % len(PHRASINGS),
            str(topics[index]["id"]),
        )

    order = sorted(range(len(topics)), key=key)
    for position, index in enumerate(order):
        topics[index]["split"] = (
            TUNING if position % TUNING_EVERY == TUNING_EVERY - 1 else HELD_OUT
        )


def _build_quality_labelled() -> Tuple[Dict[str, object], ...]:
    """The 64-query labelled set. A pure function of the tables above."""
    pairs = [
        (family, device)
        for family, devices in _QUALITY_FAMILIES
        for device in devices
    ]
    topics: List[Dict[str, object]] = []
    seen_category: Dict[str, int] = {name: 0 for name in CATEGORIES}
    for index, (family, device) in enumerate(pairs):
        category = _CATEGORY_CYCLE[index % len(_CATEGORY_CYCLE)]
        ordinal = seen_category[category]
        seen_category[category] += 1
        phrasing = PHRASINGS[ordinal % len(PHRASINGS)]
        aspect = ASPECT_ORDER[index % len(ASPECT_ORDER)]
        spec = _QUALITY_ASPECTS[aspect]
        value = spec["values"][index // len(ASPECT_ORDER)]  # type: ignore[index]
        wanted = _relevant_count(category, ordinal)
        template = (
            _VAGUE_QUERY[phrasing]
            if category == "vague"
            else spec["query"][phrasing]  # type: ignore[index]
        )
        topics.append(
            {
                "id": "%s-%s" % (family, device),
                "family": family,
                "device": device,
                "aspect": aspect,
                "value": value,
                "category": category,
                "phrasing": phrasing,
                "query": template.format(f=family, d=device),
                "facts": tuple(
                    str(part).format(f=family, d=device, v=value)
                    for part in spec["parts"][:wanted]  # type: ignore[index]
                ),
            }
        )
    for ordinal, device in enumerate(_ABSENT_DEVICES):
        family = _UNANSWERABLE_FAMILIES[ordinal]
        phrasing = PHRASINGS[ordinal % len(PHRASINGS)]
        topics.append(
            {
                "id": "%s-%s" % (family, device),
                ...
                "phrasing": phrasing,
                "query": _UNANSWERABLE_QUERY[phrasing].format(f=family, d=device),
                "facts": (),
            }
        )
    _assign_notes(topics, QUALITY_CORPUS_NOTES)
    _assign_splits(topics)
    return tuple(topics)


QUALITY_LABELLED: Tuple[Dict[str, object], ...] = _build_quality_labelled()


def labelled_counts(topics: Iterable[Dict[str, object]], field: str) -> Dict[str, int]:
    counts: Dict[str, int] = {}
    for topic in topics:
        counts[str(topic[field])] = counts.get(str(topic[field]), 0) + 1
    return dict(sorted(counts.items()))


# --- the multilingual probe -------------------------------------------------
#
# Design decision 2: the shipped embedder is all-MiniLM-L6-v2 (see
# index_toolbox.py), an English model with an English WordPiece vocabulary. A
# recall number over Cyrillic queries measures that vocabulary, not chunk
# geometry, so the multilingual half of the accepted review is a *probe* and
# never a headline. The fourth variant is what makes it evidence rather than an
# assertion: the same Russian sentence with the invented nouns left in Latin
# script. If it scores far above the fully transliterated variant, the model is
# matching a Latin token opaquely rather than reading the question.

PROBE_VARIANTS: Tuple[str, ...] = (
    "en-reference", "de", "ru-cyrillic", "uk-cyrillic", "ru-latin-anchor",
)
PROBE_BASE_QUERIES = 12

_TRANSLITERATION: Tuple[Tuple[str, str], ...] = (
    ("sch", "\u0448"), ("ch", "\u0447"), ("sh", "\u0448"), ("th", "\u0442"),
    ("ph", "\u0444"), ("kh", "\u0445"), ("ts", "\u0446"), ("yu", "\u044e"),
    ("ya", "\u044f"), ("zh", "\u0436"),
    ("a", "\u0430"), ("b", "\u0431"), ("c", "\u043a"), ("d", "\u0434"),
    ("e", "\u0435"), ("f", "\u0444"), ("g", "\u0433"), ("h", "\u0445"),
...
def to_cyrillic(text: str) -> str:
    """Mechanical Latin -> Cyrillic transliteration of an invented noun.

    Mechanical on purpose. The point of the probe is that the token the index
    holds is Latin and the token the query carries is not; a clever
    transliteration would blur the very difference being measured.
    """
    out = []
    rest = text.lower()
    while rest:
...
def probe_base_topics(
    topics: Iterable[Dict[str, object]] = QUALITY_LABELLED,
    limit: int = PROBE_BASE_QUERIES,
) -> List[Dict[str, object]]:
    """The base queries the probe varies: held-out, answerable, deterministic."""
    eligible = [
        topic
        for topic in topics
        if topic["split"] == HELD_OUT
        and topic["category"] in ("single-hop", "multi-doc")
    ]
    return sorted(eligible, key=lambda t: str(t["id"]))[:limit]


def multilingual_probe(
    topics: Iterable[Dict[str, object]] = QUALITY_LABELLED,
) -> List[Dict[str, object]]:
    """12 base queries x 5 variants, as a flat list of scorable entries."""
    entries: List[Dict[str, object]] = []
    for topic in probe_base_topics(topics):
        spec = _QUALITY_ASPECTS[str(topic["aspect"])]
        family = str(topic["family"])
        device = str(topic["device"])
        latin = "%s %s" % (family, device)
        cyrillic = "%s %s" % (to_cyrillic(family), to_cyrillic(device))
        texts = {
            "en-reference": str(spec["query"]["natural"]).format(  # type: ignore[index]
                f=family, d=device
            ),
            "de": str(spec["de"]).format(n=latin),
            "ru-cyrillic": str(spec["ru"]).format(n=cyrillic),
            "uk-cyrillic": str(spec["uk"]).format(n=cyrillic),
            "ru-latin-anchor": str(spec["ru"]).format(n=latin),
        }
        for variant in PROBE_VARIANTS:
            entries.append(
                {
                    "id": "%s::%s" % (topic["id"], variant),
                    "base_id": topic["id"],
                    "variant": variant,
                    "script": "cyrillic" if "cyrillic" in variant else "latin",
                    "entity_script": (
                        "cyrillic" if variant in ("ru-cyrillic", "uk-cyrillic") else "latin"
                    ),
                    "category": topic["category"],
                    "split": topic["split"],
                    "query": texts[variant],
                    "relevant": [note_name(i) for i in topic["notes"]],  # type: ignore
                }
            )
    return entries
...
# --- corpus geometry --------------------------------------------------------
#
# The generator was written against module constants. Phase 2 lifts the geometry
# into a value so that a second corpus can reuse every line of it - and so that
# the first corpus keeps producing byte-identical output, which is what hard
# guard 1 turns on.


class CorpusGeometry:
    """Everything build_corpus() needs that is not the target directory."""

    def __init__(
        self,
        identifier: str,
        notes: int,
        sections_per_note: int,
        seed: int,
        words: List[str],
        topics: Tuple[Dict[str, object], ...],
        scratch_note: Optional[str],
        schema_version: int,
        labelled: bool = False,
    ) -> None:
        self.identifier = identifier
        self.notes = notes
        self.sections_per_note = sections_per_note
        self.seed = seed
        self.words = words
        self.topics = topics
    ...
    def describe(self) -> Dict[str, object]:
        return {
            "id": self.identifier,
            "notes": self.notes,
            "sections_per_note": self.sections_per_note,
            "seed": self.seed,
            "vocabulary_words": len(self.words),
            "mutable_document": self.scratch_note,
            "planting_slots": self.notes * self.sections_per_note,
        }


def _timed_topics() -> Tuple[Dict[str, object], ...]:
    """QUALITY_TOPICS in the shape build_corpus() now reads.

    ``facts`` is the same sentence once per document, which is exactly what the
    timed corpus already contained - the field is added, the bytes are not
    touched.
    """
    out = []
    for topic in QUALITY_TOPICS:
        notes = tuple(topic["notes"])  # type: ignore[arg-type]
        out.append(
            dict(
                topic,
                family=str(topic["id"]).split("-")[0],
                device=str(topic["id"]).split("-")[1],
                notes=notes,
                facts=tuple(str(topic["fact"]) for _ in notes),
            )
        )
    return tuple(out)


TIMED_GEOMETRY = CorpusGeometry(
    identifier="timed",
    notes=CORPUS_NOTES,
    sections_per_note=SECTIONS_PER_NOTE,
    seed=CORPUS_SEED,
    words=_WORDS,
    topics=_timed_topics(),
    scratch_note=SCRATCH_NOTE,
    schema_version=CORPUS_SCHEMA_VERSION,
)

QUALITY_GEOMETRY = CorpusGeometry(
    identifier="quality",
    notes=QUALITY_CORPUS_NOTES,
    sections_per_note=QUALITY_SECTIONS_PER_NOTE,
    seed=QUALITY_CORPUS_SEED,
    words=QUALITY_WORDS,
    topics=QUALITY_LABELLED,
    scratch_note=None,
    schema_version=QUALITY_CORPUS_SCHEMA_VERSION,
    labelled=True,
)


def _paragraph(det: _Det, target: int, vocabulary: Optional[List[str]] = None) -> str:
    vocabulary = _WORDS if vocabulary is None else vocabulary
    words: List[str] = []
    size = 0
    while size < target:
        word = det.pick(vocabulary)
        words.append(word)
        size += len(word) + 1
        if len(words) % 14 == 0:
            words.append("\n")
...
def _note_sections(
    det: _Det, sections: int, vocabulary: Optional[List[str]] = None
) -> List[List[str]]:
    ...
    for index in range(sections):
        ...
        heading = "%s %d" % (det.pick(vocabulary), index)
        out.append([heading, _paragraph(det, target, vocabulary)])
    return out
...
def _note_text(
    det: _Det, title: str, sections: int, vocabulary: Optional[List[str]] = None
) -> str:
    return _render_note(title, _note_sections(det, sections, vocabulary))
...
def _plant_fact(sections: List[List[str]], position: int, fact: str) -> None:
    ...
    passage or diluted into a wall of neighbouring text - which is what this
    bench exists to detect.
    """
...
def quality_ground_truth(
    ...
    """query -> relevant source files, plus the three phase-2 label fields.

    Same shape as ``ground_truth()`` and the same guarantee: nothing here
    consults an index, a collection or a retriever.
    """
    labels: List[Dict[str, object]] = []
    for topic in topics:
        labels.append(
...
def build_corpus(
    target: Path, geometry: "CorpusGeometry" = TIMED_GEOMETRY
    ...
    ``geometry`` is the only thing that differs between the two corpora. It
    defaults to the timed one, whose bytes are frozen: parameterising this
    function is not allowed to move a single character of ``bench/corpus/``.
    """
    target.mkdir(parents=True, exist_ok=True)
    det = _Det(geometry.seed)
    total = 0
    planting: List[Dict[str, object]] = []
    for index in range(geometry.notes):
        name = note_name(index)
        sections = _note_sections(det, geometry.sections_per_note, geometry.words)
        ...
        slot = 0
        for topic in geometry.topics:
            note_indices = tuple(topic["notes"])  # type: ignore[arg-type]
            ...
            # Multi-document topics plant a *different part* of one answer in
            # each of their documents, so "requires combining sources" is true of
            # the corpus and not only of the label.
            fact = str(topic["facts"][note_indices.index(index)])  # type: ignore[index]
            _plant_fact(sections, position, fact)
            planting.append(
                {
                    "topic": topic["id"],
                    "source": name,
        ...
        total += len(text)
    if geometry.scratch_note is not None:
        scratch = scratch_note_text(0)
        (target / geometry.scratch_note).write_text(scratch, encoding="utf-8")
        total += len(scratch)
    ...
    # Kept, not deleted, for a geometry with no mutable note: the guard is then
    # vacuously true, which is a different thing from being absent.
    for entry in planting:
        if geometry.scratch_note is not None and entry["source"] == geometry.scratch_note:
            raise RuntimeError("a labelled fact was planted in the mutable note")

    info = {
        "files": geometry.notes + (1 if geometry.scratch_note else 0),
        "chars": total,
        "scratch": geometry.scratch_note,
...
def corpus_documents(geometry: "CorpusGeometry" = TIMED_GEOMETRY) -> List[str]:
    """Every markdown file the corpus consists of, in stable order."""
    names = [note_name(i) for i in range(geometry.notes)]
    if geometry.scratch_note is not None:
        names.append(geometry.scratch_note)
    return names


def _quality_ground_truth_document(
    planting: List[Dict[str, object]],
    files: Dict[str, bytes],
    geometry: "CorpusGeometry",
) -> dict:
    """The phase-2 labelled document: categories, splits, phrasings, probe.

    A separate builder rather than extra keys on the timed one. The timed
    corpus is frozen byte for byte (hard guard 1) and its ground-truth file is
    part of that freeze, so its document cannot grow a field - not even a null
    one.
    """
    labels = quality_ground_truth(geometry.topics)
    by_category_and_split: Dict[str, Dict[str, int]] = {}
    for entry in labels:
        bucket = by_category_and_split.setdefault(str(entry["category"]), {})
        bucket[str(entry["split"])] = bucket.get(str(entry["split"]), 0) + 1
    probe = multilingual_probe(geometry.topics)
    return {
        "schema_version": geometry.schema_version,
        "generator": "bench.py:build_corpus",
        "seed": geometry.seed,
        "geometry": geometry.describe(),
        "documents": corpus_documents(geometry),
        "mutable_document": None,
        "mutable_document_note": (
            "this corpus has none: no timed operation touches it, so nothing "
            "rewrites or removes a document while it is being scored"
        ),
        ...
        "language": "English only; see multilingual_probe for the coverage probe",
        "categories": labelled_counts(geometry.topics, "category"),
        "phrasings": labelled_counts(geometry.topics, "phrasing"),
        "splits": labelled_counts(geometry.topics, "split"),
        "counts_by_category_and_split": {
            name: dict(sorted(by_category_and_split[name].items()))
            for name in sorted(by_category_and_split)
        },
        "split_rule": (
            "queries ordered by category, then by phrasing rotated by the "
            "category's own position, then by id; every %dth entry in that order "
            "is tuning and the rest are held-out. Deterministic, exactly a "
            "quarter of every category and exactly a quarter of every phrasing, "
            "and stable across a rebuild - a split that moved would invalidate "
            "every comparison against an earlier run" % TUNING_EVERY
        ),
        "headline_split": HELD_OUT,
        "headline_rule": (
            "the profile comparison is computed over split == %r answerable "
            "queries and states its own N; the tuning subset is reported beside "
            "it, never instead of it" % HELD_OUT
        ),
        "relevant_set_contract": {
            "single-hop": "exactly 1 relevant document",
            "multi-doc": "2 or 3, and each carries a different part of the answer",
            "vague": "1 or 2, underspecified phrasing, still answerable",
            "unanswerable": "0; must score exactly zero on every rung and budget",
        },
        "queries": labels,
        "topics": [
            {
                ...
                "split": topic["split"],
                "query": topic["query"],
                "facts": list(topic["facts"]),  # type: ignore[arg-type]
                "notes": [note_name(i) for i in topic["notes"]],  # type: ignore
            }
            for topic in geometry.topics
        ],
        "unanswerable": {
            "device_nouns": list(_ABSENT_DEVICES),
            "construction": (
                "a family noun the corpus is full of, paired with a device noun "
                "that occurs nowhere in it: the retriever answers confidently "
                "and every rung must still be zero"
            ),
            "queries": [
                entry["query"] for entry in labels if entry["category"] == "unanswerable"
            ],
        },
        "multilingual_probe": {
            "purpose": (
                "measures the shipped English embedder's vocabulary coverage, "
                "not chunk geometry; never aggregated into a profile ranking"
            ),
            "variants": list(PROBE_VARIANTS),
            "base_queries": len(probe_base_topics(geometry.topics)),
            "entity_transliteration": (
                "mechanical Latin -> Cyrillic; ru-latin-anchor keeps the "
                "invented nouns in Latin script so the two Russian variants "
                "differ only in the script of the entity"
            ),
            "queries": probe,
        },
        "planting": planting,
        "digest": {
            ...
            "files": {name: _short_digest(files[name]) for name in corpus_documents(geometry)},
        },
    }
...
def ground_truth_document(
    planting: List[Dict[str, object]],
    files: Dict[str, bytes],
    geometry: "CorpusGeometry" = TIMED_GEOMETRY,
) -> dict:
    """The labelled query set, as it is written to disk beside the corpus."""
    if geometry.labelled:
        return _quality_ground_truth_document(planting, files, geometry)
    return {
        "schema_version": geometry.schema_version,
        "generator": "bench.py:build_corpus",
        "seed": CORPUS_SEED,
...
def _generate_corpus_bytes(
    geometry: "CorpusGeometry" = TIMED_GEOMETRY,
) -> Tuple[Dict[str, bytes], dict, List[Dict[str, object]]]:
    ...
    try:
        info, planting = build_corpus(scratch, geometry)
        files = {name: (scratch / name).read_bytes() for name in corpus_documents(geometry)}
    finally:
        shutil.rmtree(scratch, ignore_errors=True)
    return files, info, planting


def emit_corpus(target: Path, geometry: "CorpusGeometry" = TIMED_GEOMETRY) -> dict:
    """Write the shipped corpus and its ground truth to ``target``."""
    files, info, planting = _generate_corpus_bytes(geometry)
    target.mkdir(parents=True, exist_ok=True)
    for name, data in files.items():
        (target / name).write_bytes(data)
    document = ground_truth_document(planting, files, geometry)
...
def verify_corpus(source: Path, geometry: "CorpusGeometry" = TIMED_GEOMETRY) -> dict:
    ...
    expected, info, planting = _generate_corpus_bytes(geometry)
    problems: List[str] = []
    ...
    if not truth_path.is_file():
        ...
        if stored is not None:
            fresh = ground_truth_document(planting, expected, geometry)
            if stored != fresh:
                differing = sorted(
                    key
...
def stage_corpus(
    source: Path, scratch: Path, geometry: "CorpusGeometry" = TIMED_GEOMETRY
) -> None:
    ...
    for name in corpus_documents(geometry):
...
# The derangement search is bounded, and says so. The phase-1 implementation was
# unbounded backtracking over document-disjointness: instant at 8 queries, a
# plausible hang at 56 with overlapping relevant sets. Never loop unbounded, and
# never silently return a derangement that is not disjoint.
DERANGE_SEED = 20260722
DERANGE_ATTEMPT_CAP = 200000


def _candidate_order(index: int, total: int, seed: int) -> List[int]:
    """A deterministic shuffle of the candidate indices for one position."""
    order = list(range(total))
    det = _Det(seed + 7919 * (index + 1))
    for position in range(len(order) - 1, 0, -1):
        swap = det.step() % (position + 1)
        order[position], order[swap] = order[swap], order[position]
    return order


def _bounded_derangement(
    sets: List[set], seed: int, attempt_cap: int
) -> Tuple[Optional[List[int]], int]:
    """Seeded, attempt-capped search for a document-disjoint derangement."""
    total = len(sets)
    if total < 2:
        return None, 0
    used = [False] * total
    assignment: List[int] = [-1] * total
    orders = [_candidate_order(index, total, seed) for index in range(total)]
    attempts = [0]

    def search(index: int) -> bool:
        if index == total:
            return True
        for candidate in orders[index]:
            attempts[0] += 1
            if attempts[0] > attempt_cap:
                return False
    ...
    return (list(assignment) if search(0) else None), attempts[0]
...
def group_aggregate(
    ...
    **kwargs,
    ...
    """The same aggregate, computed once per value of ``field``.

    A sibling of the overall aggregate, never a replacement for it: the existing
    keys stay exactly where they are so nothing downstream has to know this
    exists. Each group carries its own ``queries`` count, which is what lets the
    document say the N of every cell it prints.
    """
    ...
    for name in sorted(groups):
        aggregate = dict(aggregator(groups[name], *args, **kwargs))
        aggregate["indicative_only"] = len(groups[name]) < INDICATIVE_MIN_QUERIES
        out[name] = aggregate
    return out
...
def derange_labels(
    ...
    seed: int = DERANGE_SEED,
    attempt_cap: int = DERANGE_ATTEMPT_CAP,
) -> Tuple[Optional[List[int]], Dict[str, object]]:
    """A permutation giving every query someone else's documents.

    Requires the swapped set to be *document-disjoint* from the true one, not
    merely a different query: a shuffle that happened to hand back an
    overlapping set would soften the control and make the drop look smaller than
    it is.

    Two strategies, tried in order, and which one produced the answer is
    returned so the report can say it rather than imply it:

    * ``global`` - one disjoint derangement over the whole set;
    * ``within-group`` - a disjoint derangement inside each group (the labelled
      set passes its categories), used when the global search exhausts its
      attempt cap.

    Returns ``(permutation, detail)``. ``permutation`` is ``None`` when neither
    strategy finds one, in which case the caller reports the residual overlap
    rather than pretending there is none.
    """
    total = len(relevant_sets)
    sets = [set(entry) for entry in relevant_sets]
    permutation, attempts = _bounded_derangement(sets, seed, attempt_cap)
    detail: Dict[str, object] = {
        "attempt_cap": attempt_cap,
        "seed": seed,
        "attempts": attempts,
        "strategy": "global",
        "exhausted": permutation is None,
    }
    if permutation is not None:
        return permutation, detail

    if groups is None:
        detail["strategy"] = "none"
        return None, detail

    buckets: Dict[str, List[int]] = {}
    for index, name in enumerate(groups):
        buckets.setdefault(str(name), []).append(index)
    combined: List[int] = [-1] * total
    group_attempts = 0
    for name in sorted(buckets):
        members = buckets[name]
        local, used_attempts = _bounded_derangement(
            [sets[index] for index in members], seed + len(name), attempt_cap
        )
        group_attempts += used_attempts
        if local is None:
            detail["strategy"] = "none"
            detail["attempts"] = attempts + group_attempts
            detail["failed_group"] = name
            return None, detail
        for position, index in enumerate(members):
            combined[index] = members[local[position]]
    detail["strategy"] = "within-group"
    detail["attempts"] = attempts + group_attempts
    detail["groups"] = sorted(buckets)
...
class ProfileBench:
    def __init__(
        ...
        quality_corpus: Optional[Path] = None,
        quality_truth: Optional[Dict[str, object]] = None,
    ) -> None:
        ...
        # Phase 2. The labelled set lives in a second, larger corpus, so it gets
        # a second collection. Untimed throughout: no latency sample in this
        # report has ever seen it.
        self.quality_collection_name = self.collection_name + "_quality"
        # A second, independently built copy of the same corpus. The only thing
        # it is for is measuring whether the labelled scores are reproducible at
        # this corpus size; see op_retrieval_stability().
        self.quality_repeat_collection_name = self.collection_name + "_quality2"
        self.quality_corpus = quality_corpus
        self.quality_truth = quality_truth
    ...
    def _assert_target_safe(self) -> None:
        for name in (
            ...
            self.cold_collection_name,
            self.quality_collection_name,
            self.quality_repeat_collection_name,
        ):
    ...
    def _quality_collection(self):
        return get_collection(name=self.quality_collection_name)

    def _index_quality(self) -> dict:
        """Build the labelled corpus's own collection. Untimed, by construction.

        Called after every timed operation and after the cold-start probes, so
        no latency sample can contain it.
        """
        if self.quality_corpus is None:
            return {}
        return reindex(
            reset=True,
            root=self.quality_corpus,
            collection_name=self.quality_collection_name,
            config=self.config,
        )
    ...
    def op_mcp_roundtrip(self, runs: int = MCP_ROUNDTRIP_RUNS) -> dict:
        ...
        no code change — and the check below proves it took hold rather than
    ...
    def op_quality(self) -> dict:
        ...
        permutation, derangement_detail = derange_labels(relevant_sets)
    ...
    # -- the labelled set on the quality corpus (untimed) --------------------

    def _labelled_context(self) -> Tuple[list, dict, tuple, int, dict]:
        """(labels, facts-by-(topic, source), ladder, depth, topics-by-id)."""
        truth = self.quality_truth or {}
        labels = [dict(entry) for entry in truth["queries"]]  # type: ignore[index]
        topics = {str(topic["id"]): topic for topic in truth["topics"]}  # type: ignore[index]
        facts: Dict[Tuple[str, str], str] = {}
        for identifier, topic in topics.items():
            for position, source in enumerate(topic["notes"]):  # type: ignore[index]
                facts[(identifier, str(source))] = str(topic["facts"][position])  # type: ignore[index]
        ladder = tuple(truth["k_ladder"])  # type: ignore[index]
        depth = int(truth["retrieval_depth"])  # type: ignore[index]
        return labels, facts, ladder, depth, topics

    def op_labelled_quality(self) -> dict:
        """Equal-k scoring of the 64-query labelled set. Takes no timing samples.

        The headline aggregate is the held-out subset and says so. The tuning
        subset is reported beside it, never instead of it: the three profiles
        were selected while the phase-1 labelled set was visible, so a set that
        is entirely visible to profile selection cannot support a claim about
        profile selection.
        """
        collection = self._quality_collection()
        labels, facts, ladder, depth, _topics = self._labelled_context()
        corpus_files = sorted(path.name for path in (self.quality_corpus or Path(".")).glob("*.md"))
        problems: List[str] = []
        ...
        for entry in labels:
            for source in entry["relevant"]:  # type: ignore[union-attr]
                if source not in corpus_files:
                    raise RuntimeError(
                        "labelled ground truth names %r, which is not in the "
                        "quality corpus" % source
                    )

        planting = list((self.quality_truth or {}).get("planting") or [])  # type: ignore[union-attr]
        present = 0
        for entry in planting:
            key = (str(entry["topic"]), str(entry["source"]))
            fact = facts.get(key)
            stored = collection.get(where={"source": entry["source"]})
            documents = stored.get("documents") or []
            if fact is not None and any(fact in (text or "") for text in documents):
                present += 1
            else:
                problems.append(
                    "labelled fact %s not intact in any chunk of %s"
                    % (entry["topic"], entry["source"])
                )
        ...
        for entry in labels:
            ...
            result["phrasing"] = entry["phrasing"]
            scored.append(result)

        answerable = [s for s in scored if s["category"] != "unanswerable"]
        controls = [s for s in scored if s["category"] == "unanswerable"]

        # Hard guard 5, grown with the set. Every unanswerable query, every
        # recall rung, rr and nDCG, exactly zero. Eight of them now instead of
        # one; the contract is not softened by there being more of it.
        nonzero = []
        for entry in controls:
            for key, value in list(entry["recall"].items()) + [  # type: ignore[union-attr]
                ("rr", entry["rr"]),
                ("ndcg", entry["ndcg@%d" % depth]),
            ]:
                if value:
                    nonzero.append("%s/%s=%s" % (entry["id"], key, value))
        if nonzero:
            raise RuntimeError(
                "unanswerable controls scored above zero on %s - the labels are "
                "not independent of the retriever" % nonzero
            )

        # Derangement control, now enforced rather than merely reported.
        relevant_sets = [list(entry["relevant"]) for entry in answerable]  # type: ignore[arg-type]
        permutation, derangement = derange_labels(
            relevant_sets, groups=[str(entry["category"]) for entry in answerable]
        )
        shuffled: List[Dict[str, object]] = []
        overlap = 0
        if permutation is None:
            control_problems.append(
                "no document-disjoint derangement of the labelled set exists "
                "(%s)" % derangement
            )
        else:
            for index, entry in enumerate(answerable):
                ...
                overlap += len(set(swapped) & set(relevant_sets[index]))
                result = score_query(
                    list(entry["retrieved_sources"]), swapped, ladder, depth  # type: ignore[arg-type]
                )
                result["id"] = entry["id"]
                result["category"] = entry["category"]
                result["split"] = entry["split"]
                result["labelled_as"] = answerable[permutation[index]]["id"]
                shuffled.append(result)
            if overlap:
                control_problems.append(
                    "deranged labels overlap the truth in %d document(s)" % overlap
                )

        true_aggregate = aggregate_scores(answerable, ladder, depth)
        shuffled_aggregate = aggregate_scores(shuffled, ladder, depth) if shuffled else {}
        dropped = bool(
            shuffled_aggregate
            and shuffled_aggregate.get("recall@%d" % depth, 1.0)
        ...
        if not dropped:
            control_problems.append(
                "the deranged-label control did not drop: recall@%d %s -> %s. A "
                "control that does not come out low is not a control"
                % (
                    depth,
                    true_aggregate.get("recall@%d" % depth),
                    shuffled_aggregate.get("recall@%d" % depth),
                )
            )

        held_out = [s for s in answerable if s["split"] == HELD_OUT]
        tuning = [s for s in answerable if s["split"] == TUNING]

        return {
            "measured": True,
            "timed": False,
            "measured_after": "all timed ops and both cold-start probes",
            "corpus": (self.quality_corpus.name if self.quality_corpus else None),
            "collection": self.quality_collection_name,
            "metric": "recall@k, document-level; MRR; nDCG@%d" % depth,
            "k_ladder": list(ladder),
            "retrieval_depth": depth,
            "language": "English only (design decision 2); see multilingual_probe",
            "ground_truth": {
                "origin": "planted at corpus generation",
                "loaded_from": (self.quality_truth or {}).get(
                    "loaded_from", "generated in-process"
                ),
                "seed": (self.quality_truth or {}).get("seed", QUALITY_CORPUS_SEED),
                "derived_from_retriever_output": False,
                "queries": len(labels),
                "answerable_queries": len(answerable),
                "corpus_documents": len(corpus_files),
                "categories": (self.quality_truth or {}).get("categories"),
                "phrasings": (self.quality_truth or {}).get("phrasings"),
                "splits": (self.quality_truth or {}).get("splits"),
                "counts_by_category_and_split": (self.quality_truth or {}).get(
                    "counts_by_category_and_split"
                ),
                "split_rule": (self.quality_truth or {}).get("split_rule"),
                "relevant_set_contract": (self.quality_truth or {}).get(
                    "relevant_set_contract"
                ),
                "relevant_per_query": {
                    "min": min((len(s) for s in relevant_sets), default=0),
                    "max": max((len(s) for s in relevant_sets), default=0),
                    "mean": round(
                        statistics.fmean([len(s) for s in relevant_sets]), 3
                    )
                    if relevant_sets
                    else 0.0,
                },
                "facts_intact_in_index": "%d/%d" % (present, len(planting)),
            },
            "headline_split": HELD_OUT,
            "headline": aggregate_scores(held_out, ladder, depth),
            "aggregate": true_aggregate,
            "aggregate_scope": "all answerable queries, both splits",
            "by_split": group_aggregate(answerable, "split", aggregate_scores, ladder, depth),
            "by_category": group_aggregate(scored, "category", aggregate_scores, ladder, depth),
            "by_phrasing": group_aggregate(answerable, "phrasing", aggregate_scores, ladder, depth),
            "by_category_held_out": group_aggregate(
                held_out, "category", aggregate_scores, ladder, depth
            ),
            "by_category_tuning": group_aggregate(
                tuning, "category", aggregate_scores, ladder, depth
            ),
            "indicative_min_queries": INDICATIVE_MIN_QUERIES,
            "per_query": scored,
            "controls": {
                "unanswerable_queries": {
                    "purpose": "queries no document answers must score 0 everywhere",
                    "count": len(controls),
                    "all_scored_zero": True,
                    "per_query": controls,
                },
                "shuffled_ground_truth": {
                    "purpose": (
    ...
    def op_labelled_budget(self) -> dict:
        """The labelled set again under a fixed context budget. Untimed."""
        collection = self._quality_collection()
        labels, _facts, _ladder, _depth, _topics = self._labelled_context()
        total = collection.count()
        depth = max(1, min(total, BUDGET_RETRIEVAL_CAP))
        ...
        for budget in self.budgets:
            ...
            controls = [s for s in scored if s["category"] == "unanswerable"]
            nonzero = [
                "%s=%s" % (entry["id"], entry["recall@budget"])
                for entry in controls
                if entry["recall@budget"]
            ]
            if nonzero:
                raise RuntimeError(
                    "unanswerable controls scored recall@budget %s at budget %d "
                    "- the labels are not independent of the retriever"
                    % (nonzero, budget)
                )
            held_out = [s for s in answerable if s["split"] == HELD_OUT]
            by_budget[str(budget)] = {
                "headline_split": HELD_OUT,
                "headline": aggregate_budget(held_out),
                "aggregate": aggregate_budget(answerable),
                "aggregate_scope": "all answerable queries, both splits",
                "by_split": group_aggregate(answerable, "split", aggregate_budget),
                "by_category": group_aggregate(scored, "category", aggregate_budget),
                "by_category_held_out": group_aggregate(
                    held_out, "category", aggregate_budget
                ),
                "by_phrasing": group_aggregate(answerable, "phrasing", aggregate_budget),
                "per_query": scored,
                "controls_unanswerable": {
                    "count": len(controls),
                    "all_scored_zero": True,
                    "per_query": controls,
                },
    ...
    def op_retrieval_stability(self, first_pass: List[Dict[str, object]]) -> dict:
        """Is a labelled score reproducible when the index is rebuilt?

        The timed corpus is 48-256 vectors and its scores come out identical run
        after run. The quality corpus is several times larger, which is enough
        for the vector store's approximate nearest-neighbour structure to be in
        play, and an approximate index built twice can order near-ties
        differently. That is not a defect to hide behind a rounded number: it is
        measured here, on a second independently built copy of the same corpus,
        and the result is published beside the scores it qualifies.
        """
        if self.quality_corpus is None or self.quality_truth is None:
            return {}
        reindex(
            reset=True,
            root=self.quality_corpus,
            collection_name=self.quality_repeat_collection_name,
            config=self.config,
        )
        collection = get_collection(name=self.quality_repeat_collection_name)
        labels, _facts, ladder, depth, _topics = self._labelled_context()

        second: List[Dict[str, object]] = []
        differing = 0
        for entry, previous in zip(labels, first_pass):
            ranked = self._sources_in_results(collection, str(entry["query"]), depth)
            if ranked != list(previous["retrieved_sources"]):  # type: ignore[arg-type]
                differing += 1
            result = score_query(ranked, entry["relevant"], ladder, depth)  # type: ignore[arg-type]
            result["id"] = entry["id"]
            result["category"] = entry["category"]
        ...
        answerable_first = [s for s in first_pass if s["category"] != "unanswerable"]
        answerable_second = [s for s in second if s["category"] != "unanswerable"]
        held_first = [s for s in answerable_first if s["split"] == HELD_OUT]
        held_second = [s for s in answerable_second if s["split"] == HELD_OUT]
        aggregate_first = aggregate_scores(held_first, ladder, depth)
        aggregate_second = aggregate_scores(held_second, ladder, depth)
        deltas = {
            key: round(float(aggregate_second[key]) - float(aggregate_first[key]), 6)
            for key in aggregate_first
            if key != "queries"
        }
        changed_recall = sum(
            1 for a, b in zip(first_pass, second) if a["recall"] != b["recall"]
        )
        return {
            "measured": True,
            "timed": False,
            "method": (
                "the same corpus indexed a second time into a separate "
                "collection, the same labelled queries re-issued against it, and "
                "the two ranked lists compared position by position"
            ),
            "collection": self.quality_repeat_collection_name,
            "queries": len(labels),
            "rankings_differing": differing,
            "rankings_identical": len(labels) - differing,
            "queries_whose_recall_changed": changed_recall,
            "headline_split": HELD_OUT,
            "headline_first_build": aggregate_first,
            "headline_second_build": aggregate_second,
            "headline_deltas": deltas,
            "max_abs_headline_delta": (
                round(max(abs(value) for value in deltas.values()), 6) if deltas else 0.0
            ),
        }

    def op_multilingual_probe(self) -> dict:
        """Embedder coverage, not chunk geometry. Never a headline.

        The shipped embedder is an English model with an English WordPiece
        vocabulary, so a Cyrillic query is measuring that vocabulary. The fourth
        variant is what turns the observation into evidence: the same Russian
        sentence with the invented nouns left in Latin script. If it scores far
        above the fully transliterated one, the model is matching a Latin token
        opaquely rather than reading the question.
        """
        collection = self._quality_collection()
        truth = self.quality_truth or {}
        block = truth.get("multilingual_probe") or {}
        entries = list(block.get("queries") or [])  # type: ignore[union-attr]
        ladder = tuple(truth["k_ladder"])  # type: ignore[index]
        depth = int(truth["retrieval_depth"])  # type: ignore[index]
        ...
        for entry in entries:
            ...
            result = score_query(ranked, entry["relevant"], ladder, depth)  # type: ignore[arg-type]
            result["id"] = entry["id"]
            result["base_id"] = entry["base_id"]
            result["variant"] = entry["variant"]
            result["entity_script"] = entry["entity_script"]
            result["query"] = entry["query"]
            scored.append(result)

        by_variant = group_aggregate(scored, "variant", aggregate_scores, ladder, depth)
        return {
            "measured": bool(scored),
            "timed": False,
            "excluded_from_headline": True,
            "excluded_from_profile_ranking": True,
            "measures": (
                "the vocabulary coverage of the embedder this toolkit ships "
                "(all-MiniLM-L6-v2, an English model with an English WordPiece "
                "vocabulary), not chunk geometry"
            ),
            "embedder": (self.profile.get("embedder") or {}).get("model"),
            "base_queries": block.get("base_queries"),
            "variants": list(PROBE_VARIANTS),
            "entity_transliteration": block.get("entity_transliteration"),
            "queries": len(scored),
            "k_ladder": list(ladder),
            "retrieval_depth": depth,
    ...
    def run(self) -> dict:
        ...
        # Phase 2, on the second corpus and its own collection. Also strictly
        # untimed: the index build below happens after every latency sample this
        # profile will report.
        labelled_quality: Dict[str, object] = {}
        labelled_budget: Dict[str, object] = {}
        probe: Dict[str, object] = {}
        stability: Dict[str, object] = {}
        quality_index: Dict[str, object] = {}
        if self.quality_corpus is not None and self.quality_truth is not None:
            quality_index = self._index_quality()
            labelled_quality = self.op_labelled_quality()
            labelled_budget = self.op_labelled_budget()
            probe = self.op_multilingual_probe()
            stability = self.op_retrieval_stability(
                list(labelled_quality.get("per_query") or [])  # type: ignore[arg-type]
            )
        ...
        return {
            ...
            "labelled_quality": labelled_quality,
            "labelled_token_budget": labelled_budget,
            "multilingual_probe": probe,
            "retrieval_stability": stability,
            "quality_corpus_index": quality_index,
            "read_stages": stages,
...
def render_table(report: dict) -> str:
    ...
    lines.append("")
    lines.append(render_labelled(report.get("labelled_quality"), report.get("multilingual_probe")))
    lines.append("=" * 78)
    return "\n".join(lines)
...
def render_labelled(labelled: Optional[dict], probe: Optional[dict]) -> str:
    """Console view of the 64-query labelled set.

    Aggregates only. One row per labelled query was readable at 8 and is a wall
    at 64, and the per-query detail is in the result JSON either way - a console
    dump is not where anybody reads it.
    """
    if not labelled:
        return "labelled set: not measured (no --corpus-quality given)"
    truth = labelled["ground_truth"]
    ladder = labelled["k_ladder"]
    depth = labelled["retrieval_depth"]
    ndcg_key = "ndcg@%d" % depth
    columns = ["recall@%d" % k for k in ladder] + ["mrr", ndcg_key]

    lines: List[str] = []
    lines.append(
        "labelled set (untimed, corpus %s): %s queries, %s answerable, %s documents"
        % (
            labelled.get("corpus"),
            truth["queries"],
            truth["answerable_queries"],
            truth["corpus_documents"],
        )
    )
    lines.append(
        "  categories %s / splits %s / phrasings %s; facts intact %s"
        % (
            truth.get("categories"),
            truth.get("splits"),
            truth.get("phrasings"),
            truth["facts_intact_in_index"],
        )
    )
    header = "  %-30s %5s %s" % ("", "n", " ".join("%9s" % c for c in columns))
    lines.append("")
    lines.append(header)
    lines.append("  " + "-" * (len(header) - 2))

    def row(label: str, block: dict) -> str:
        cells = [
            "        -" if block.get(c) is None else "%9.3f" % block[c] for c in columns
        ]
        return "  %-30s %5s %s" % (
            label[:30],
            block.get("queries", "-"),
    ...
    lines.append(
        row("HEADLINE (%s)" % labelled["headline_split"], labelled["headline"])
    )
    for name, block in labelled["by_split"].items():
        lines.append(row("split %s" % name, block))
    for name, block in labelled["by_category"].items():
        lines.append(
            row(
                "category %s%s" % (name, " *" if block.get("indicative_only") else ""),
                block,
            )
        )
    for name, block in labelled["by_phrasing"].items():
        lines.append(row("phrasing %s" % name, block))
    shuffled = labelled["controls"]["shuffled_ground_truth"]
    if shuffled["aggregate"]:
        lines.append(row("CONTROL deranged labels", shuffled["aggregate"]))
    lines.append("")
    lines.append(
        "  controls: %s unanswerable queries, all zero at every rung; deranged "
        "labels overlap %s, dropped %s, strategy %s"
        % (
            labelled["controls"]["unanswerable_queries"]["count"],
            shuffled["document_overlap_with_truth"],
            shuffled["dropped"],
            shuffled["search"]["strategy"],
        )
    )
    lines.append(
        "  * indicative only: fewer than %d queries" % labelled["indicative_min_queries"]
    )
    for problem in labelled.get("control_problems") or []:
        lines.append("  CONTROL PROBLEM: %s" % problem)
    for problem in labelled.get("problems") or []:
        lines.append("  PROBLEM: %s" % problem)

    if probe and probe.get("measured"):
        lines.append("")
        lines.append(
            "multilingual probe (untimed, embedder coverage, never a headline):"
        )
        lines.append(header)
        lines.append("  " + "-" * (len(header) - 2))
        for name, block in probe["by_variant"].items():
            lines.append(row("variant %s" % name, block))
    return "\n".join(lines)
...
def load_results(directory: Path, profiles: Iterable[str], stamp: Optional[str] = None) -> List[dict]:
    ...
    if missing:
        raise RuntimeError(
            ...
            "'python bench.py --all --corpus bench/corpus "
            "--corpus-quality bench/corpus-quality' first"
...
def render_benchmarks(reports: List[dict]) -> str:
    ...
    lines.append(
        ...
        "--corpus-quality bench/corpus-quality"
    )
    lines.append("")
    lines.append(
    ...
    lines.extend(
        _table(
            ...
            [
                ...
                [
                    "corpus (timed)",
                    "`bench/corpus/` — %s files, %s characters, seed %s, %s "
                    "planted facts. **Every timed figure in this document comes "
            ...
            + (
                [
                    [
                        "corpus (labelled, untimed)",
                        "`bench/corpus-quality/` — %s files, %s characters, seed "
                        "%s, %s planted facts, %s x %s planting slots. **No timed "
                        "operation touches it**; it exists because the labelled "
                        "set needs more planting slots than the timed corpus has, "
                        "and growing the timed corpus would move every latency "
                        "figure above."
                        % (
                            (harness.get("quality_corpus") or {}).get("files"),
                            (harness.get("quality_corpus") or {}).get("chars"),
                            (harness.get("quality_corpus") or {}).get("seed"),
                            (harness.get("quality_corpus") or {}).get("planted_facts"),
                            ((harness.get("quality_corpus") or {}).get("geometry") or {}).get("notes"),
                            ((harness.get("quality_corpus") or {}).get("geometry") or {}).get("sections_per_note"),
                        ),
                    ]
                ]
                if harness.get("quality_corpus")
                else []
            )
            + [
                [
    ...
    # --- the labelled set (phase 2) -----------------------------------------
    # A second corpus, a second labelled set, and every table below states its
    # split. Nothing here is timed and nothing here touches bench/corpus.
    labelled0 = ordered[0].get("labelled_quality") or {}
    if labelled0:
        ltruth = labelled0["ground_truth"]
        lladder = labelled0["k_ladder"]
        ldepth = labelled0["retrieval_depth"]
        lndcg = "ndcg@%d" % ldepth
        lcolumns = ["recall@%d" % k for k in lladder] + ["mrr", lndcg]
        indicative_min = int(labelled0.get("indicative_min_queries", INDICATIVE_MIN_QUERIES))
        counts = ltruth.get("counts_by_category_and_split") or {}
        qcorpus = (harness.get("quality_corpus") or {})

        def labelled_of(report: dict) -> dict:
            return report.get("labelled_quality") or {}

        def _mark(block: dict) -> str:
            return " \u2020" if block.get("indicative_only") else ""

        def _score_rows(picker) -> List[List[str]]:
            rows = []
            for report in ordered:
                block = picker(labelled_of(report))
                rows.append(
                    [report["profile"]["name"], str(block.get("queries", "-"))]
                    + [_fmt(block.get(column), 3) for column in lcolumns]
                )
            return rows

        lines.append("## Retrieval quality \u2014 the labelled set")
        lines.append("")
        lines.append(
            "Phase 1 scored retrieval over eight queries on the timed corpus. "
            "One miss there is a 12.5 point swing, and `recall@budget` came out "
            "saturated at 1.000 on two of the three profiles, so the comparison "
            "resolved the bottom of the field and not the top. This section is "
            "the replacement: **%s labelled queries** (%s answerable, %s "
            "unanswerable controls) over a **second, larger generated corpus** "
            "of %s documents and %s characters, seed %s. **The timed corpus is "
            "untouched by any of it** \u2014 every latency figure above is a "
            "property of `bench/corpus`, which this section never reads."
            % (
                ltruth.get("queries"),
                ltruth.get("answerable_queries"),
                (ltruth.get("categories") or {}).get("unanswerable"),
                ltruth.get("corpus_documents"),
                qcorpus.get("chars"),
                qcorpus.get("seed"),
            )
        )
        lines.append("")
        lines.append(
            "Every query carries a **category**, a **split** and a **phrasing**. "
            "**Untimed throughout, so no mode applies to any table in this "
            "section.** The counts, which are read out of the shipped "
            "ground-truth file rather than typed here:"
        )
        lines.append("")
        lines.extend(
            _table(
                ...
                [
                    [
                        name,
                        str(sum((counts.get(name) or {}).values())),
                        str((counts.get(name) or {}).get(HELD_OUT, 0)),
                        str((counts.get(name) or {}).get(TUNING, 0)),
                        str((ltruth.get("relevant_set_contract") or {}).get(name, "?")),
                    ]
        ...
        lines.append("")
        lines.append(
            "Phrasing is balanced across the answerable set \u2014 %s \u2014 and covers "
            "how people actually ask: a full question (`natural`), a keyword "
            "fragment with no sentence structure (`terse`), a conversational ask "
            "with filler the retriever has to see past (`verbose`), and one "
            "plausible typo in a content word, never in the invented entity noun "
            "(`misspelled`). A typo in the invented noun would test exact match "
            "and nothing else."
            % ", ".join(
                "%s %s" % (value, key)
                for key, value in sorted((ltruth.get("phrasings") or {}).items())
            )
        )
        lines.append("")
        lines.append("### The headline: held-out queries only")
        lines.append("")
        lines.append(
            "The three profiles already existed and were selected while the "
            "phase-1 eight-query set was visible. A labelled set that is "
            "entirely visible to profile selection cannot support a claim about "
            "profile selection, so the set carries a declared split and **the "
            "headline is the held-out subset**. The rule, as recorded in the "
            "shipped ground-truth file: %s." % ltruth.get("split_rule")
        )
        lines.append("")
        lines.append(
            "**N = %s held-out answerable queries. Untimed, so no mode applies.**"
            % (labelled0.get("headline") or {}).get("queries")
        )
        lines.append("")
        lines.extend(_table(["profile", "n"] + lcolumns, _score_rows(lambda b: b.get("headline") or {})))
        lines.append("")
        lines.append(
            "`recall@1` is a live discriminator here and was not one in phase 1. "
            "Every phase-1 query had two or three relevant documents, which caps "
            "the mean `recall@1` by construction; %s of the queries in this set "
            "have **exactly one** relevant document, so on that subset one "
            "retrieved chunk can reach 1.000."
            % ((ltruth.get("categories") or {}).get("single-hop"))
        )
        lines.append("")
        lines.append("### Held-out against tuning")
        lines.append("")
        lines.append(
            "Both are reported; only the first is the headline. If the two "
            "disagree about the ranking, that disagreement is itself a finding "
            "and is stated below the table. **Untimed; each row states its own N.**"
        )
        lines.append("")
        for split_name in (HELD_OUT, TUNING):
            block0 = (labelled0.get("by_split") or {}).get(split_name) or {}
            lines.append(
                "Split **%s** \u2014 n = %s answerable queries%s:"
                % (
                    split_name,
                    block0.get("queries"),
                    ", **indicative only, fewer than %d queries**" % indicative_min
                    if block0.get("indicative_only")
                    else "",
                )
            )
            lines.append("")
            lines.extend(
                _table(
                    ["profile", "n"] + lcolumns,
                    _score_rows(
                        lambda b, s=split_name: (b.get("by_split") or {}).get(s) or {}
                    ),
                )
            )
            lines.append("")

        def _rank_by(picker, metric: str) -> List[str]:
            scores = {
                report["profile"]["name"]: float(
                    (picker(labelled_of(report)) or {}).get(metric) or 0.0
                )
                for report in ordered
            }
            return sorted(scores, key=lambda n: (-scores[n], n))

        held_rank = _rank_by(lambda b: b.get("by_split", {}).get(HELD_OUT), "recall@3")
        tune_rank = _rank_by(lambda b: b.get("by_split", {}).get(TUNING), "recall@3")
        if held_rank == tune_rank:
            lines.append(
                "The two splits agree on the ordering by `recall@3`: %s. That "
                "agreement is worth stating rather than assuming \u2014 it is the "
                "reason the held-out number can be read as the profile's "
                "behaviour and not as an artefact of which queries landed in "
                "which subset." % " > ".join("`%s`" % n for n in held_rank)
            )
        else:
            lines.append(
                "**The two splits disagree about the ranking.** Held-out orders "
                "the profiles %s by `recall@3`; the tuning subset orders them "
                "%s. The held-out ordering is the one this document reports, "
                "because it is the one the profiles were not selected against \u2014 "
                "and the disagreement is itself the finding: at this N the "
                "ordering is not stable across a resampling of the same query "
                "population."
                % (
                    " > ".join("`%s`" % n for n in held_rank),
                    " > ".join("`%s`" % n for n in tune_rank),
                )
            )
        lines.append("")

        lines.append("### By category, held out")
        lines.append("")
        lines.append(
            "What *kind* of query separates the profiles, if any does. **Untimed; "
            "every row states its own N, and a row computed from fewer than %d "
            "queries is marked \u2020 and is indicative only \u2014 it is printed so the "
            "shape is visible, not so it can be quoted.**" % indicative_min
        )
        lines.append("")
        category_names = sorted(
            (labelled0.get("by_category_held_out") or {}), key=str
        )
        for category in category_names:
            block0 = (labelled0.get("by_category_held_out") or {}).get(category) or {}
            lines.append(
                "Category **%s**, held out \u2014 n = %s%s:"
                % (
                    category,
                    block0.get("queries"),
                    " \u2020 **indicative only**" if block0.get("indicative_only") else "",
                )
            )
            lines.append("")
            lines.extend(
                _table(
                    ["profile", "n"] + lcolumns,
                    _score_rows(
                        lambda b, c=category: (b.get("by_category_held_out") or {}).get(c)
                        or {}
                    ),
                )
            )
        ...
        lines.append("### By phrasing")
        lines.append("")
        lines.append(
            "Both splits, answerable queries only. **Untimed; each row states "
            "its own N.** This is the one table in the document that says "
            "anything about *how the question was asked* rather than about the "
            "index."
        )
        lines.append("")
        for phrasing in sorted(labelled0.get("by_phrasing") or {}):
            block0 = (labelled0.get("by_phrasing") or {}).get(phrasing) or {}
            lines.append(
                "Phrasing **%s** \u2014 n = %s%s:"
                % (
                    phrasing,
            ...
            lines.extend(
                _table(
                    ["profile", "n"] + lcolumns,
                    _score_rows(
                        lambda b, ph=phrasing: (b.get("by_phrasing") or {}).get(ph) or {}
                    ),
                )
            )
            lines.append("")

        stability0 = ordered[0].get("retrieval_stability") or {}
        if stability0 and stability0.get("measured"):
            lines.append("### Are these scores reproducible?")
            lines.append("")
            lines.append(
                "The timed corpus is small enough that its retrieval scores come "
                "out identical run after run. This one is not, and rather than "
                "round the question away it is measured: %s. **Untimed; n = %s "
                "labelled queries per profile.**"
                % (stability0.get("method"), stability0.get("queries"))
            )
            lines.append("")
            rows = []
            for report in ordered:
                block = report.get("retrieval_stability") or {}
                rows.append(
                    [
                        report["profile"]["name"],
                        str(block.get("queries")),
                        str(block.get("rankings_identical")),
                        str(block.get("rankings_differing")),
                        str(block.get("queries_whose_recall_changed")),
                        _fmt(block.get("max_abs_headline_delta"), 6),
                    ]
                )
            lines.extend(
                _table(
                    [
                        ...
                        "identical rankings",
                        "differing rankings",
                        "queries whose recall changed",
                        "largest headline shift",
                    ],
                    rows,
                )
            )
            ...
            worst = max(
                (
                    float((r.get("retrieval_stability") or {}).get("max_abs_headline_delta") or 0.0)
                    for r in ordered
                ),
                default=0.0,
            )
            total_differing = sum(
                int((r.get("retrieval_stability") or {}).get("rankings_differing") or 0)
                for r in ordered
            )
            if total_differing == 0:
                lines.append(
                    "Every ranked list was reproduced exactly on every profile, "
                    "so nothing in the labelled tables above depends on which "
                    "build of the index answered."
                )
            else:
                lines.append(
                    "Some ranked lists are **not** reproduced exactly: %d across "
                    "the three profiles. The cause is the vector store's "
                    "approximate index, which this corpus is large enough to "
                    "engage and the timed corpus is not \u2014 two builds can order "
                    "near-ties differently. What it costs is measured rather "
                    "than assumed: the largest shift in any held-out headline "
                    "metric on any profile is **%s**. Read every labelled "
                    "figure in this document as carrying at least that much "
                    "reproduction noise, and treat a gap of that size as no "
                    "gap.\n\nAt least, and not exactly: this is **one rebuild "
                    "inside one run on one machine**, so it is a lower bound on "
                    "run-to-run variation and not a ceiling. A separate run of "
                    "the same code over the same committed corpora is free to "
                    "move a labelled figure further than this. The timed "
                    "corpus's own equal-k quality block, by contrast, does "
                    "reproduce exactly \u2014 it is small enough that the search is "
                    "not approximate at all."
                    % (total_differing, _fmt(worst, 6))
                )
            lines.append("")

        lines.append("### Controls on the labelled set")
        lines.append("")
        lines.append(
            "**Untimed, so again no mode applies.** Two controls, and both are "
            "enforced rather than reported. A non-zero unanswerable score "
            "raises before anything is written at all. A deranged-label set "
            "that fails to drop, or that overlaps the truth, sets a non-zero "
            "exit the way a state-guard problem does \u2014 the result file for "
            "that profile exists, and this document is not regenerated from it."
        )
        lines.append("")
        rows = []
        for report in ordered:
            block = labelled_of(report)
            controls = block.get("controls") or {}
            unanswerable = controls.get("unanswerable_queries") or {}
            shuffled = controls.get("shuffled_ground_truth") or {}
            rows.append(
                [
                    report["profile"]["name"],
                    str(unanswerable.get("count")),
                    "0.000 at every rung" if unanswerable.get("all_scored_zero") else "NON-ZERO",
                    str(shuffled.get("document_overlap_with_truth")),
                    str((shuffled.get("search") or {}).get("strategy")),
                    "%s \u2192 %s"
                    % (
                        _fmt((block.get("aggregate") or {}).get("recall@3"), 3),
                        _fmt((shuffled.get("aggregate") or {}).get("recall@3"), 3),
                    ),
                ]
            )
        lines.extend(
        ...
        lines.append("")
        lines.append(
            "The %s unanswerable queries are built the same way the phase-1 "
            "control was: a family noun the corpus is full of, paired with a "
            "device noun that occurs nowhere in it. The retriever answers them "
            "confidently with lexically plausible chunks, and every rung, `rr` "
            "and `nDCG` must still be exactly zero. The derangement search is "
            "seeded and attempt-capped rather than unbounded, and the strategy "
            "it used is reported above instead of being assumed."
            % (ltruth.get("categories") or {}).get("unanswerable")
        )
        lines.append("")

        # --- the labelled set under a budget ---------------------------------
        lbudget0 = ordered[0].get("labelled_token_budget") or {}
        if lbudget0:
            lkeys = sorted((lbudget0.get("by_budget") or {}), key=int)
            lines.append("## The labelled set under a token budget")
            lines.append("")
            lines.append(
                "The same %s labelled queries, scored again with the *context* "
                "held fixed instead of the chunk count. **Untimed; means over "
                "held-out answerable queries; tokens are `ceil(len(text) / 4)`, "
                "an approximation, see Method.**" % ltruth.get("queries")
            )
            lines.append("")
            for key in lkeys:
                block0 = ((lbudget0.get("by_budget") or {}).get(key) or {})
                head0 = block0.get("headline") or {}
                lines.append(
                    "Budget **%s tokens**, split **%s** \u2014 **untimed, n = %s "
                    "queries:**" % (key, HELD_OUT, head0.get("queries"))
                )
                lines.append("")
                rows = []
                for report in ordered:
                    block = (
                        ((report.get("labelled_token_budget") or {}).get("by_budget") or {})
                        .get(key)
                        or {}
                    )
                    aggregate = block.get("headline") or {}
                    rows.append(
                        [
                            report["profile"]["name"],
                            str(aggregate.get("queries", "-")),
                            _fmt(aggregate.get("recall@budget"), 3),
                            _fmt(aggregate.get("unique_docs"), 2),
                            _fmt(aggregate.get("chunks_admitted"), 2),
                            _fmt(aggregate.get("tokens_used"), 1),
                ...
                lines.append("")
                for category in sorted(block0.get("by_category_held_out") or {}):
                    cat0 = (block0.get("by_category_held_out") or {}).get(category) or {}
                    lines.append(
                        "Budget %s, category **%s**, held out \u2014 n = %s%s:"
                        % (
                            key,
                            category,
                            cat0.get("queries"),
                            " \u2020 **indicative only**"
                            if cat0.get("indicative_only")
                            else "",
                        )
                    )
                    lines.append("")
                    ...
                    for report in ordered:
                        block = (
                            (
                                (report.get("labelled_token_budget") or {}).get("by_budget")
                                or {}
                            ).get(key)
                            or {}
                        )
                        aggregate = (block.get("by_category_held_out") or {}).get(
                            category
                        ) or {}
                        rows.append(
                            [
                    ...
                    lines.append("")
            lines.append(
                "All %s unanswerable controls were run under every budget on "
                "every profile and scored `recall@budget` 0 in all of them; a "
                "non-zero score there fails the run, exactly as it does at equal k."
                % (ltruth.get("categories") or {}).get("unanswerable")
            )
            lines.append("")

        # --- the multilingual probe ------------------------------------------
        probe0 = ordered[0].get("multilingual_probe") or {}
        if probe0 and probe0.get("measured"):
            lines.append("## Multilingual probe")
            lines.append("")
            lines.append(
                "**This is not a profile comparison and is never aggregated into "
                "one.** It measures %s. **Untimed; n = %s base queries \u00d7 %s "
                "variants = %s scored queries per profile**, all drawn from the "
                "held-out `single-hop` and `multi-doc` sets."
                % (
                    probe0.get("measures"),
                    probe0.get("base_queries"),
                    len(probe0.get("variants") or []),
                    probe0.get("queries"),
                )
            )
            lines.append("")
            lines.append(
                "The external review this v2 answers asked for an EN+DE+RU+UK "
                "labelled mix. "
                "The embedder this toolkit ships is `%s`, an English model with "
                "an English WordPiece vocabulary, so a recall number over "
                "Cyrillic queries measures that vocabulary and not chunk "
                "geometry \u2014 which is why the labelled set above is English only "
                "and this is a probe. The fourth variant is what makes it "
                "evidence rather than an assertion: `ru-latin-anchor` is the "
                "same Russian sentence as `ru-cyrillic` with the invented entity "
                "nouns left in Latin script, so the two differ in exactly one "
                "thing." % probe0.get("embedder")
            )
            lines.append("")
            for report in ordered:
                probe = report.get("multilingual_probe") or {}
                lines.append(
                    "Profile `%s` \u2014 **untimed, n = %s queries per variant:**"
                    % (report["profile"]["name"], PROBE_BASE_QUERIES)
                )
                lines.append("")
                rows = []
                for variant in probe.get("variants") or []:
                    block = (probe.get("by_variant") or {}).get(variant) or {}
                    rows.append(
                        [variant, str(block.get("queries", "-"))]
                        + [_fmt(block.get(column), 3) for column in lcolumns]
                    )
                lines.extend(_table(["variant", "n"] + lcolumns, rows))
                lines.append("")

            def _probe_metric(report: dict, variant: str, metric: str) -> Optional[float]:
                block = ((report.get("multilingual_probe") or {}).get("by_variant") or {}).get(
                    variant
                ) or {}
                value = block.get(metric)
                return float(value) if value is not None else None

            lines.append("### What the probe found")
            lines.append("")
            lines.append(
                "The comparison the probe exists for, **per profile rather than "
                "averaged**: a mean over three profiles would hide a profile "
                "that went the other way, and one of the three things this "
                "document is for is not doing that. `anchor \u2212 cyrillic` is the "
                "cost of transliterating the invented noun and nothing else \u2014 "
                "the Russian sentence around it is identical."
            )
            lines.append("")
            probe_metrics = ("recall@3", "mrr")
            rows = []
            for report in ordered:
                row = [report["profile"]["name"]]
                for metric in probe_metrics:
                    anchor = _probe_metric(report, "ru-latin-anchor", metric)
                    cyr = _probe_metric(report, "ru-cyrillic", metric)
                    english = _probe_metric(report, "en-reference", metric)
                    row.extend(
                        [
                            _fmt(english, 3),
                            _fmt(anchor, 3),
                            _fmt(cyr, 3),
                            _fmt(None if anchor is None or cyr is None else anchor - cyr, 3),
                        ]
                    )
                rows.append(row)
            headers = ["profile"]
            for metric in probe_metrics:
                headers.extend(
                    [
                        ...
                        "%s anchor" % metric,
                        "%s cyrillic" % metric,
                        "%s anchor \u2212 cyrillic" % metric,
                    ]
                )
            lines.extend(_table(headers, rows))
            lines.append("")

            # A profile whose English reference is itself near the floor cannot
            # resolve this comparison at all, and saying so is part of the
            # result rather than a caveat bolted onto it.
            floor = 0.25
            resolving, at_floor = [], []
            for report in ordered:
                english = _probe_metric(report, "en-reference", "recall@3")
                (at_floor if english is None or english < floor else resolving).append(
                    report["profile"]["name"]
                )
            verdicts = {}
            for metric in probe_metrics:
                gaps = {
                    report["profile"]["name"]: (
                        (_probe_metric(report, "ru-latin-anchor", metric) or 0.0)
                        - (_probe_metric(report, "ru-cyrillic", metric) or 0.0)
                    )
                    for report in ordered
                    if report["profile"]["name"] in resolving
                }
                verdicts[metric] = gaps
            positive = {
                metric: [name for name, gap in gaps.items() if gap > 0]
                for metric, gaps in verdicts.items()
            }
            unanimous = all(
                len(positive[metric]) == len(verdicts[metric]) and verdicts[metric]
                for metric in probe_metrics
            )
            none_positive = all(not positive[metric] for metric in probe_metrics)

            if at_floor:
                lines.append(
                    "**%s cannot resolve this comparison and %s excluded from the "
                    "verdict below.** Its own English reference scores %s "
                    "`recall@3` on these twelve questions, which is at the floor: "
                    "a profile that cannot answer the question in English tells "
                    "you nothing about what transliterating the noun costs. That "
                    "is a fact about %s retrieval on this corpus \u2014 see the "
                    "labelled-set tables above \u2014 not about the probe."
                    % (
                        " and ".join("`%s`" % n for n in at_floor),
                        "is" if len(at_floor) == 1 else "are",
                        ", ".join(
                            _fmt(_probe_metric(r, "en-reference", "recall@3"), 3)
                            for r in ordered
                            if r["profile"]["name"] in at_floor
                        ),
                        " and ".join("`%s`" % n for n in at_floor),
                    )
                )
            ...
            if unanimous:
                lines.append(
                    "**The Latin entity noun is doing the work, not the "
                    "question.** On every profile that can resolve it (%s), and "
                    "on both metrics, the same Russian sentence scores higher "
                    "with the invented noun left in Latin than with it "
                    "transliterated: %s. The model is matching a Latin proper "
                    "noun as an opaque token rather than reading the question, "
                    "and a Cyrillic query loses that anchor."
                    % (
                        ", ".join("`%s`" % n for n in resolving),
                        "; ".join(
                            "%s on `%s` %+0.3f" % (metric, name, gap)
                            for metric in probe_metrics
                            for name, gap in sorted(verdicts[metric].items())
                        ),
                    )
                )
            elif none_positive:
                lines.append(
                    "**The script of the entity noun did not help.** On no "
                    "resolving profile and on neither metric did leaving the "
                    "invented noun in Latin score above transliterating it: %s. "
                    "Whatever separates the English and Russian variants here, "
                    "the probe's evidence does not point at the entity token."
                    % "; ".join(
                        "%s on `%s` %+0.3f" % (metric, name, gap)
                        for metric in probe_metrics
                        for name, gap in sorted(verdicts[metric].items())
                ...
                lines.append(
                    "**The anchor helps, but not unanimously, and this document "
                    "reports that rather than averaging it away.** Leaving the "
                    "invented noun in Latin scores above transliterating it on "
                    "%d of the %d resolving profile-metric pairs: %s. The "
                    "direction is the expected one and the evidence for it is "
                    "partial \u2014 a mean over the profiles would have printed a "
                    "single confident number and hidden every disagreement "
                    "inside it."
                    % (
                        sum(len(positive[metric]) for metric in probe_metrics),
                        sum(len(verdicts[metric]) for metric in probe_metrics),
                        "; ".join(
                            "%s on `%s` %+0.3f" % (metric, name, gap)
                            for metric in probe_metrics
                            for name, gap in sorted(verdicts[metric].items())
            ...
            lines.append("")
            lines.append(
                "What this does **not** say: nothing here is a statement about "
                "chunking, and no number in this section enters any profile "
                "ranking, any `by_category` mean or any headline. Swapping in a "
                "multilingual embedder is a different comparison and is not in "
                "this run."
            )
            lines.append("")
    ...
    # --- headline 4: does the larger held-out set separate the top? ---------
    # Phase 1 had to call baseline and chunk-small a tie: eight queries, and
    # recall@budget pinned at 1.000 on both. This is the question that run could
    # not answer, and every branch below is implemented so the data picks one -
    # including the branch in which the answer is still no.
    labelled_reports = {
        r["profile"]["name"]: (r.get("labelled_quality") or {}) for r in ordered
    }
    labelled_budgets = {
        r["profile"]["name"]: (r.get("labelled_token_budget") or {}) for r in ordered
    }
    pair = [name for name in ("baseline", "chunk-small") if labelled_reports.get(name)]
    if len(pair) == 2 and all(labelled_reports[name] for name in pair):
        left, right = pair
        head_left = labelled_reports[left].get("headline") or {}
        head_right = labelled_reports[right].get("headline") or {}
        n_held = int(head_left.get("queries") or 0)
        # A gap is not a separation unless it clears both of the two things
        # that can produce one for free: what a single query can move the mean
        # at this N, and what rebuilding the index moves it by. Both are
        # measured, neither is chosen.
        sampling_floor = (1.0 / n_held) if n_held else 1.0
        reproduction_floor = max(
            (
                float(
                    (report.get("retrieval_stability") or {}).get(
                        "max_abs_headline_delta"
                    )
                    or 0.0
                )
        ...
        noise_floor = max(sampling_floor, reproduction_floor)
        rungs = ["recall@%d" % k for k in lladder] + ["mrr", lndcg] if labelled0 else []

        def _gap(block_left: dict, block_right: dict, metric: str) -> Optional[float]:
            a, b = block_left.get(metric), block_right.get(metric)
        ...
        equal_k_separating = [
            (metric, gap)
            for metric in rungs
            for gap in [_gap(head_left, head_right, metric)]
            if gap is not None and abs(gap) > noise_floor
        ]
        category_separating = []
        for category in sorted(labelled_reports[left].get("by_category_held_out") or {}):
            cat_left = (labelled_reports[left].get("by_category_held_out") or {}).get(category) or {}
            cat_right = (labelled_reports[right].get("by_category_held_out") or {}).get(category) or {}
            cat_n = int(cat_left.get("queries") or 0)
            if not cat_n:
                continue
            cat_floor = 1.0 / cat_n
            for metric in rungs:
                gap = _gap(cat_left, cat_right, metric)
                if gap is not None and abs(gap) > cat_floor:
                    category_separating.append(
                        (category, metric, gap, cat_n, bool(cat_left.get("indicative_only")))
                    )
        budget_separating = []
        for key in sorted((labelled_budgets[left].get("by_budget") or {}), key=int):
            bl = ((labelled_budgets[left].get("by_budget") or {}).get(key) or {}).get("headline") or {}
            br = ((labelled_budgets[right].get("by_budget") or {}).get(key) or {}).get("headline") or {}
            gap = _gap(bl, br, "recall@budget")
            if gap is not None and abs(gap) > noise_floor:
                budget_separating.append((key, gap, int(bl.get("queries") or 0)))

        # Phase 1's verdict on the same pair, read out of the timed-corpus block
        # in these same files rather than remembered.
        phase1_left = float((_quality_row(
            next(r for r in ordered if r["profile"]["name"] == left)
        ) or {}).get("recall@3") or 0.0)
        phase1_right = float((_quality_row(
            next(r for r in ordered if r["profile"]["name"] == right)
        ) or {}).get("recall@3") or 0.0)
        phase1_dir = (
            "=" if abs(phase1_left - phase1_right) <= _TIE
            else (">" if phase1_left > phase1_right else "<")
        )
        head_gap3 = _gap(head_left, head_right, "recall@3") or 0.0
        phase2_dir = (
            "=" if abs(head_gap3) <= noise_floor
            else (">" if head_gap3 > 0 else "<")
        )
        reversed_ranking = (
            phase1_dir != "=" and phase2_dir != "=" and phase1_dir != phase2_dir
        )

        if reversed_ranking:
            heading = "The larger held-out set reverses the phase-1 ranking"
        elif equal_k_separating and budget_separating:
            heading = "The larger held-out set separates the two profiles phase 1 tied"
        elif equal_k_separating:
            heading = (
                "The larger held-out set separates the two profiles at equal k, "
                "and a token budget closes the gap again"
            )
        elif budget_separating:
            heading = (
                "The two profiles are indistinguishable at equal k and separate "
                "only under a token budget"
            )
        elif category_separating:
            heading = (
                "No overall separation, but one category of query does separate "
                "the two profiles"
            )
        else:
            heading = (
                "The larger held-out set still does not separate the two "
                "profiles phase 1 tied"
            )
        lines.append("### 4. %s" % heading)
        lines.append("")
        lines.append(
            "Phase 1 could not answer this: eight labelled queries, one miss "
            "worth 12.5 points, and `recall@budget` saturated at 1.000 on both "
            "`%s` and `%s`. The comparison below is over **%d held-out "
            "answerable queries on the quality corpus, untimed**, and a gap is "
            "called a separation only when it clears **%.4f** \u2014 the larger of "
            "what one query can move the mean at this N (%.4f) and what "
            "rebuilding the index moves a headline metric by (%.4f, measured "
            "above). Both are computed, neither is chosen."
            % (left, right, n_held, noise_floor, sampling_floor, reproduction_floor)
        )
        lines.append("")
        rows = []
        for metric in rungs:
            gap = _gap(head_left, head_right, metric)
            rows.append(
                [
                    metric,
                    _fmt(head_left.get(metric), 4),
                    _fmt(head_right.get(metric), 4),
                    _fmt(gap, 4),
                    "separated" if gap is not None and abs(gap) > noise_floor else "within noise",
                ]
            )
        lines.extend(
            _table(
                ["metric", "`%s`" % left, "`%s`" % right, "difference", "verdict"],
                rows,
            )
        )
        lines.append("")
        ...
        if reversed_ranking:
            lines.append(
                "**The ranking reverses.** On the timed corpus's eight-query "
                "set, `%s` %s `%s` by `recall@3` (%.4f against %.4f). On %d "
                "held-out queries over the larger corpus the order is the other "
                "way (%.4f against %.4f). Two labelled sets over two corpora "
                "disagree about which of these profiles is better, and that is "
                "the finding: at eight queries the phase-1 ordering was not a "
                "property of the profiles.\n\nTwo things changed at once and the "
                "reversal cannot be attributed to either alone: the labelled set "
                "grew from 8 queries to %d held-out ones, **and** it moved to a "
                "different corpus with more notes and longer ones. What can be "
                "said is narrower and still worth saying \u2014 the phase-1 "
                "ordering survived neither change, so it was not robust enough "
                "to choose a profile on. Separating the two causes needs this "
                "labelled set run against the timed corpus, and its %d planting "
                "slots cannot hold it."
                % (
                    left, phase1_dir, right, phase1_left, phase1_right,
                    n_held,
                    float(head_left.get("recall@3") or 0.0),
                    float(head_right.get("recall@3") or 0.0),
                    n_held,
                    CORPUS_NOTES * SECTIONS_PER_NOTE,
                )
            )
        elif equal_k_separating:
            lines.append(
                "**They separate.** %s. The separation appears at equal k, so "
                "it is not an artefact of how the context budget is packed."
                % "; ".join(
                    "`%s` leads by %.4f on %s" % (left if gap > 0 else right, abs(gap), metric)
                    for metric, gap in equal_k_separating
                )
            )
            if budget_separating:
                lines.append("")
                lines.append(
                    "It survives a token budget as well: %s."
                    % "; ".join(
                        "at %s tokens the gap is %.4f in favour of `%s` over %d "
                        "held-out queries"
                        % (key, abs(gap), left if gap > 0 else right, count)
                        for key, gap, count in budget_separating
                    )
                )
            else:
                lines.append("")
                lines.append(
                    "It does **not** survive a token budget: at every budget "
                    "measured the two are within %.4f of each other. Equal k "
                    "hands the coarser profile more text for the same nominal "
                    "depth, so a separation that exists only there is a "
                    "separation in how much context was spent, not in how well "
                    "it was chosen." % noise_floor
                )
        elif budget_separating:
            lines.append(
                "**They are indistinguishable at equal k and separate only "
                "under a budget.** Every equal-k metric above is inside the "
                "%.4f noise floor, while %s. Equal k is not a fair comparison "
                "across chunk geometries, and this is what that costs: the two "
                "profiles look identical until the constraint is context."
                % (
                    noise_floor,
                    "; ".join(
                        "at %s tokens `%s` leads by %.4f over %d held-out queries"
                        % (key, left if gap > 0 else right, abs(gap), count)
                        for key, gap, count in budget_separating
                    ),
                )
            )
            ...
            lines.append(
                "**No overall separation, but the categories are not "
                "unanimous.** Every aggregate metric above is inside the noise "
                "floor, and yet %s. A category result is a narrower claim than "
                "the aggregate and each one states its own N above; where that N "
                "is below %d the row is marked indicative only and should be "
                "read as a direction to test, not as a result."
                % (
                    "; ".join(
                        "on `%s` queries (n=%d%s) `%s` leads by %.4f on %s"
                        % (
                            category,
                            count,
                            ", indicative only" if indicative else "",
                            left if gap > 0 else right,
                            abs(gap),
                            metric,
                        )
                        for category, metric, gap, count, indicative in category_separating[:6]
                    ),
                    indicative_min if labelled0 else INDICATIVE_MIN_QUERIES,
                )
            )
        else:
            lines.append(
                "**They still do not separate.** Over %d held-out answerable "
                "queries, at every rung of the k ladder, on MRR, on nDCG and "
                "under every token budget, `%s` and `%s` are within %.4f of each "
                "other \u2014 inside what one query can move. Phase 1 called this "
                "pair a tie on eight queries and suggested a larger labelled set "
                "would resolve it. It did not."
                % (n_held, left, right, noise_floor)
            )
            lines.append("")
            lines.append(
                "The honest reading is that **more queries are not what would "
                "separate these two profiles on this corpus.** Multiplying the "
                "labelled set by eight moved the metric off its ceiling \u2014 "
                "`recall@3` is %.4f and %.4f here rather than the 1.000 phase 1 "
                "reported \u2014 so the set is no longer saturated and the two "
                "profiles are still the same. What would separate them is a "
                "*different corpus*: one whose sections are long enough that a "
                "%s-character budget and a %s-character budget actually cut the "
                "text differently. On a body this shape they largely do not."
                % (
                    float(head_left.get("recall@3") or 0.0),
                    float(head_right.get("recall@3") or 0.0),
                    (next(r for r in ordered if r["profile"]["name"] == left)["profile"].get("chunking") or {}).get("max_chars"),
                    (next(r for r in ordered if r["profile"]["name"] == right)["profile"].get("chunking") or {}).get("max_chars"),
                )
            )
        lines.append("")
        lines.append(
            "Whatever the branch above says, it is a statement about **%d "
            "held-out queries on one synthetic corpus, untimed**. It is not a "
            "statement about latency, about a real knowledge base, or about any "
            "corpus larger than this one." % n_held
        )
        lines.append("")
    ...
    for item in [
        ...
        [
            "**Two corpora, and only one of them is timed.** Every latency, "
            "cold-start and stage-split figure in this document is a property of "
            "the %s-document timed corpus. Every figure in the labelled-set "
            "sections is a property of the %s-document quality corpus, which no "
            "timed operation touches. The two must not be read across: a "
            "retrieval score there says nothing about the latency tables here."
            % (corpus.get("files"), (labelled0.get("ground_truth") or {}).get("corpus_documents")),
            "**Synthetic prose, real phrasings.** The labelled facts are "
            "generated from a small set of sentence templates over invented "
            "entity nouns, so the corpus can be shipped and re-derived byte for "
            "byte and so the labels cannot have leaked from anything a model saw "
            "in pretraining. Realism lives on the *query* side \u2014 terse "
            "fragments, verbose asks, misspellings \u2014 not in the text being "
            "retrieved. Real notes have different heading density and different "
            "vocabulary overlap, and this bench does not measure that.",
            "**The labelled set is English only.** By choice, not by oversight: "
            "the embedder is an English model, so a Cyrillic recall number would "
            "measure its vocabulary rather than chunk geometry. The multilingual "
            "half of the accepted review is reported as a probe in its own "
            "section and never enters a ranking. Nothing here says how a "
            "multilingual embedder would behave.",
            "**A held-out split mitigates tuning exposure; it does not undo "
            "it.** The three profiles existed before this labelled set did, so "
            "the held-out subset is genuinely unseen by profile selection \u2014 "
            "but the *corpus generator* was written by the same hand as the "
            "profiles, and a synthetic body cannot be blind in the way a real "
            "one would be.",
            "**The labelled scores carry reproduction noise, and the figure "
            "for it is a lower bound.** The quality corpus is large enough to "
            "engage the vector store's approximate index, so rebuilding it can "
            "reorder near-ties. Measured, not assumed \u2014 see *Are these scores "
            "reproducible?* \u2014 and the largest shift one rebuild caused in any "
            "held-out headline metric on any profile is %s. That is one rebuild "
            "in one run: a separate run may move a labelled figure further. "
            "Only differences well clear of it should be read as differences. "
            "The timed corpus is too small for any of this to arise, which is "
            "why the equal-k quality block on that corpus reproduces exactly."
            % _fmt(
                max(
                    (
                        float(
                            (r.get("retrieval_stability") or {}).get(
                                "max_abs_headline_delta"
                            )
                            or 0.0
                        )
            ...
            "**%d held-out answerable queries is still %d queries.** One query "
            "moves the mean by %.4f. Every separation claim in this document is "
            "tested against that figure, and a gap smaller than it is reported "
            "as noise rather than as a result."
            % (
                int(((labelled0.get("headline") or {}).get("queries")) or 0),
                int(((labelled0.get("headline") or {}).get("queries")) or 0),
                1.0 / max(1, int(((labelled0.get("headline") or {}).get("queries")) or 1)),
            ),
        ]
        if labelled0
        else []
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    parser.add_argument(
        ...
        metavar="DIR",
        help=(
            "write the larger labelled quality corpus and its ground truth to "
            "DIR, then exit. Never one of the timed corpus directories"
        ),
    )
    parser.add_argument(
        "--corpus-quality",
        ...
        metavar="DIR",
        help=(
            "score the labelled set against a quality corpus already on disk "
            "(verified against the generator byte for byte)"
        ),
    )
    parser.add_argument(
        "--render-benchmarks",
    ...
    # A corpus directory may not stand in for the other one: the two carry
    # different geometries and verify_corpus would report a wall of diffs that
    # says nothing about either.
    for left, right in (
        ("--corpus", args.corpus), ("--emit-corpus", args.emit_corpus),
    ):
        for other_name, other in (
            ("--corpus-quality", args.corpus_quality),
            ("--emit-corpus-quality", args.emit_corpus_quality),
        ):
            if right is not None and other is not None and Path(right) == Path(other):
                parser.error(
                    "%s and %s must not name the same directory: the timed and "
                    "the quality corpus have different geometries"
                    % (left, other_name)
                )

    # --- corpus emission: a generator run, not a measurement ----------------
    if args.emit_corpus_quality is not None:
        summary = emit_corpus(args.emit_corpus_quality, QUALITY_GEOMETRY)
        if not args.quiet:
            print(
                "quality corpus written to %s: %d files, %d characters, "
                "%d planted facts, %d labelled queries"
                % (
                    summary["dir"],
                    len(summary["files"]),
                    summary["chars"],
                    summary["planted_facts"],
                    len(QUALITY_LABELLED),
                )
            )
        return 0
    ...
    quality_corpus = RAG_ROOT / (".bench-corpus-quality-%d" % os.getpid())
    shipped: Optional[Path] = args.corpus
    shipped_before: Optional[str] = None
    shipped_quality: Optional[Path] = args.corpus_quality
    shipped_quality_before: Optional[str] = None
    quality_truth: Optional[Dict[str, object]] = None
    quality_staged: Optional[Path] = None
    benched: List[str] = []
    written: List[Path] = []
    ...
    try:
        ...
        # The quality corpus is read-only for the whole run: nothing timed
        # touches it, so it is verified, staged once and never written again.
        if shipped_quality is not None:
            shipped_quality_before = _fingerprint_tree(shipped_quality)
            quality_info = verify_corpus(shipped_quality, QUALITY_GEOMETRY)
            quality_truth = dict(
                load_ground_truth(shipped_quality),
                loaded_from=shipped_quality.name + "/" + GROUND_TRUTH_FILE,
            )
            stage_corpus(shipped_quality, quality_corpus, QUALITY_GEOMETRY)
            quality_staged = quality_corpus
            if not args.quiet:
                print(
                    "quality corpus verified: %s matches the generator byte for "
                    "byte (%d files, %d characters, %d labelled queries)"
                    % (
                        shipped_quality,
                        quality_info["files"],
                        quality_info["chars"],
                        len(quality_truth["queries"]),  # type: ignore[arg-type]
                    )
                )
        ...
        for name in profile_names:
            ...
            bench = ProfileBench(
                ...
                truth=truth,
                budgets=budgets,
                quality_corpus=quality_staged,
                quality_truth=quality_truth,
            )
            benched.append(bench.collection_name)
            benched.append(bench.cold_collection_name)
            benched.append(bench.quality_collection_name)
            benched.append(bench.quality_repeat_collection_name)
            ...
            report = {
                ...
                "harness": {
                    ...
                    "quality_corpus": (
                        dict(
                            ...
                            kind="synthetic",
                            seed=QUALITY_CORPUS_SEED,
                            geometry=QUALITY_GEOMETRY.describe(),
                            source=(
                                "shipped: %s (verified against the generator)"
                                % shipped_quality.as_posix()
                            ),
                            used_for="untimed labelled scoring only; no timed "
                ...
                # Phase 2: the categorised, split labelled set on the second
                # corpus. Untimed, like everything above it in this block.
                "labelled_quality": result["labelled_quality"],
                "labelled_token_budget": result["labelled_token_budget"],
                # Embedder coverage. Never aggregated into a profile ranking.
                "multilingual_probe": result["multilingual_probe"],
                # Whether a labelled score survives the index being rebuilt.
                "retrieval_stability": result["retrieval_stability"],
                # Verdicts only: see StateGuard.public().
                "state_guard": guard.public(),
            }
            report["table"] = render_table(report)

            # A control that only reports is not a control. The deranged-label
            # control was a boolean nobody acted on until now; a set that does
            # not drop, or overlaps the truth, fails the run the same way a
            # StateGuard problem does.
            for problem in (result["labelled_quality"] or {}).get("control_problems") or []:
                control_failures.append("%s: %s" % (name, problem))
        ...
        shutil.rmtree(quality_corpus, ignore_errors=True)
        if control_failures:
            status = 1
            print("CONTROL FAILED:", file=sys.stderr)
            for problem in control_failures:
        ...
        for label, directory, before in (
            ("shipped corpus", shipped, shipped_before),
            ("shipped quality corpus", shipped_quality, shipped_quality_before),
        ):
            if directory is None or before is None:
                continue
            after = _fingerprint_tree(directory)
            if after != before:
                status = 1
                print(
                    "SHIPPED CORPUS MUTATED: %s changed during the run (%s -> %s)"
                    % (directory, before[:16], after[:16]),
                    file=sys.stderr,
                )
            elif not args.quiet:
                print("%s unchanged: %s" % (label, directory))
